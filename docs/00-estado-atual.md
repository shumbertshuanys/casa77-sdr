...
(2026-09-07, Python 3.14.5) — **três execuções listadas, todas aprovadas**, realizadas **na
entrega funcional**, sobre os bytes que vieram a ser exatamente os blobs integrados, e
**reexecutadas com os mesmos números imediatamente antes do merge**:
...
108. **O PARSER PURO DA GRAMÁTICA DE `caminho_yaml` ESTÁ INTEGRADO À `main` PELO PR #131.**
     ...
     **Python 3.14.5**, sob **`-W error`**, com **zero failures, zero errors e zero warnings** e
     **nenhum teste preexistente alterado, removido ou pulado**. As três execuções foram
     **medidas na entrega funcional, antes do merge**, e **reexecutadas com os mesmos números
     imediatamente antes do merge**. **A suíte é inteiramente SINTÉTICA** e **nenhum teste lê o
     corpus versionado**. **Estes números são evidência da ENTREGA FUNCIONAL do PR #131, e NÃO
