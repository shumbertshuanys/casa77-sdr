...
Atualizado em: 2026-09-08 (**reconciliação documental pós-merge da validação estrutural
contextual de `caminho_yaml` — PR #133**). Esta entrega é **EXCLUSIVAMENTE DOCUMENTAL**: altera
...
**Baseline funcional corrente: `4941 passed` / Python 3.14.5.**

**Execuções da PR #133, registro da validação estrutural contextual de `caminho_yaml`**
...
Existem agora **vinte e seis entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #133** ela é a **validação
   estrutural contextual de `caminho_yaml`**, **como fronteira isolada**, em
   `src/casa77_sdr/response_yaml_path_context.py`, registrada no **item 109** — que **não** é
   denominada `C15`, `E15` nem `C-A6`, e que **ainda NÃO está integrada a `E1`**; a **entrega
   funcional imediatamente anterior** é o **parser puro e determinístico da gramática de
   `caminho_yaml`**, em `src/casa77_sdr/response_yaml_path.py` (PR #131), registrado no
...
2. Commit funcional atual: `3097e61b61a9577c82301eaa0279f754f50c63f5` (PR #133). Merge
   correspondente: `78158257571065efaf950b6bbaf377bb4be40d8f`. O par
   `cee95ebb6a725c3b91595589591fefa650d64196` / `6ae5cd829bb6eb9f4acfc60a061ee9b7e6d36180`
   (PR #131), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`4941 passed`**, com **`200 passed`** no teste direcionado de
   `tests/test_response_yaml_path_context.py` e **`347 passed`** no conjunto parser +
   contexto, em **Python 3.14.5** — **zero failures e zero errors**, e **zero warnings** sob
   `-W error`. Baseline anterior integrado: **`4741 passed`**; delta **+200**, correspondente
   exatamente ao arquivo direcionado **novo**, **sem alteração de teste preexistente**. As
   execuções — **`200`**, **`347`** e **`4941`**, todas sob `-W error` — foram **medidas na
   entrega funcional do PR #133, antes do merge**, sobre os **arquivos auditados no working
   tree em CRLF**, que **após a normalização de fim de linha CRLF → LF realizada pelo Git
   correspondem exatamente aos blobs integrados** — **sem mudança textual, lógica ou
   funcional**, e **sem alegação de identidade byte-a-byte** —, e **reexecutadas com os mesmos
   números imediatamente antes do merge**; **nenhum `pytest` foi executado nesta
   reconciliação**. O registro anterior deste
   item — **`4741 passed`** / **`147 passed`** do PR #131 — **permanece correto como registro
