...
CI/CHECKS — NÃO FALHA DE CI**. **A C14 NÃO** aplica status ao corpus, **NÃO** altera
`knowledge/**`, **NÃO** compõe C8/C11/C12/C13, **NÃO** cria índice, **NÃO** executa a bijeção
...
**Baseline funcional corrente: `4343 passed` / Python 3.14.5.**

**Execuções da PR #120, registro da entrega C14** (2026-09-06, Python 3.14.5) — **cinco
...
Existem agora **vinte e duas entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #120** ela é a **décima
   quarta microentrega funcional de `C` — o leitor/validador determinístico de status por
   fragmento sob `PARCIAL`**, em `src/casa77_sdr/response_fragment_status.py`, registrada no
   **item 102**; a **décima terceira** é a propagação pura e determinística de status
   `ST1`–`ST3` aos fragmentos já associados, em
...
2. Commit funcional atual: `75025cc07c8d95f998c61f7516bf3db82bc9c06f` (PR #120). Merge
   correspondente: `e8db60d95c104993d657de32b80e749dee8003ef`. O par
   `cbf3e379c9b6697cd502b6a7a0bd3e9bb76c1a27` / `50c1d5e681a11923c443c1f117de5751ea846cdd`
   (PR #115), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`4343 passed`**, com **`243 passed`** no teste direcionado de
   `tests/test_response_fragment_status.py`, em **Python 3.14.5** — **zero failures e zero
   errors**, e **zero warnings** sob `-W error`. Baseline anterior integrado: **`4100
   passed`**; delta **+243**, correspondente exatamente ao arquivo direcionado **novo**,
   **sem alteração de teste preexistente**. As execuções — **`243`**, **`201`** (C8), **`299`**
   (C12) e **`4343`**, todas sob `-W error` — foram **medidas na entrega funcional C14, antes
   do merge**, sobre os bytes que vieram a ser exatamente os blobs integrados, e
   **reexecutadas com os mesmos números após a correção de docstring `ca4ec54bc8…`**;
   **nenhum `pytest` foi executado nesta reconciliação**. O registro anterior deste item —
   **`4100 passed`** / **`397 passed`** do PR #115 — **permanece correto como registro daquele
   momento**; ver a seção **Testes**.
