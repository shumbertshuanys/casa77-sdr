...
Existem agora **vinte e três entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #124** ela é a **associação
   física determinística de seção e fragmentos**, em
   `src/casa77_sdr/response_section_membership.py`, registrada no **item 104** — que **não** é
   denominada `C15`; a **décima
...
2. Commit funcional atual: `0e41165406c3a9261a1fcc8d0f9c600b0469e730` (PR #124). Merge
   correspondente: `ecaf0a2b3a594f8fedd10f708461ad2eeb18e086`. O par
   `75025cc07c8d95f998c61f7516bf3db82bc9c06f` / `e8db60d95c104993d657de32b80e749dee8003ef`
   (PR #120), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`4477 passed`**, com **`134 passed`** no teste direcionado de
   `tests/test_response_section_membership.py`, em **Python 3.14.5** — **zero failures e zero
   errors**, e **zero warnings** sob `-W error`. Baseline anterior integrado: **`4343
   passed`**; delta **+134**, correspondente exatamente ao arquivo direcionado **novo**,
   **sem alteração de teste preexistente**. As execuções — **`134`**, **`201`** (C8), **`299`**
   (C12), **`243`** (C14) e **`4477`**, todas sob `-W error` — foram **medidas na entrega
   funcional do PR #124, antes do merge**, sobre os bytes que vieram a ser exatamente os blobs
   integrados; **nenhum `pytest` foi executado nesta reconciliação**. O registro anterior deste
   item — **`4343 passed`** / **`243 passed`** do PR #120 — **permanece correto como registro
