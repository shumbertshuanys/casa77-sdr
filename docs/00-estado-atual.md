...
Atualizado em: 2026-09-06 (**reconciliação documental pós-merge da DÉCIMA TERCEIRA
MICROENTREGA FUNCIONAL DE `C` — propagação pura e determinística de status `ST1`–`ST3` aos
fragmentos já associados**). Esta entrega é **EXCLUSIVAMENTE DOCUMENTAL**: altera **somente**
...
**Baseline funcional corrente: `4100 passed` / Python 3.14.5.**

**Execuções da PR #115, registro da entrega C13** (2026-09-06, Python 3.14.5) — **três
...
Existem agora **vinte e uma entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
`src/casa77_sdr/response_status_propagation.py` —, integrada pelo **PR #115**.
**Nenhuma das treze materializa `C`**: o índice `knowledge/indice-respostas-aprovadas.yaml`
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #115** ela é a **décima
   terceira microentrega funcional de `C` — a propagação pura e determinística de status
   `ST1`–`ST3` aos fragmentos já associados**, em
   `src/casa77_sdr/response_status_propagation.py`, registrada no **item 98**; a **décima
   ...
   **Nenhuma das treze microentregas materializa
...
2. Commit funcional atual: `cbf3e379c9b6697cd502b6a7a0bd3e9bb76c1a27` (PR #115). Merge
   correspondente: `50c1d5e681a11923c443c1f117de5751ea846cdd`. O par
   `798beb31fd0dc4fed34aa7b20d397abcd2ef8c2b` / `eae7b5098b248cefb42b3a82569fc0575fd6fee0`
   (PR #113), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`4100 passed`**, com **`397 passed`** no teste direcionado de
   `tests/test_response_status_propagation.py`, em **Python 3.14.5** — **zero failures e zero
   errors**, e **zero warnings** sob `-W error`. Baseline anterior integrado: **`3703
   passed`**; delta **+397**, correspondente exatamente ao arquivo direcionado **novo**,
   **sem alteração de teste preexistente**. As execuções — **`397`** e **`4100`**, ambas sob
   `-W error` — foram **medidas na entrega funcional C13, antes do merge**, sobre os bytes
   que vieram a ser exatamente os blobs integrados; **nenhum `pytest` foi executado nesta
   reconciliação**. O registro anterior deste item — **`3703 passed`** / **`299 passed`**
   do PR #113 — **permanece correto como registro daquele momento**; ver a seção
   **Testes**.
