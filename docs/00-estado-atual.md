...
- **Fora de `C`**, o **produtor não determinístico de `N-b`** está **materializado
  localmente nesta entrega candidata**: a fronteira agnóstica de provedor
  ...
  obrigatória**, terminando em `Interpretacao`. **Não está versionada**, a
...
| **N-b** — `N-b-RES2` e integração da etapa 4 | contrato arbitrado; fronteira determinística materializada; **produtor não determinístico materializado localmente** (`docs/07` §6.3, `M-PN1`–`M-PN12`) — Anthropic / `claude-sonnet-5`, saída estruturada, canonicalização obrigatória, zero retry, exercitado **offline** pela suíte. **Não versionado**; os **evals semânticos** e o **smoke** existem e **não foram executados**. **Pendentes**: a **integração da etapa 4** no ciclo e **`N-b-RES2`**, que exige arbitragem própria | bloqueia o `OrquestradorMotor` e a integração completa | `docs/07` §6.3, §12 item 12 |
...
- **N-b** — **`N-b-RES2`** e a **integração da etapa 4** no ciclo. O **produtor não
  determinístico deixou de ser a lacuna**: ele está materializado localmente
  (`docs/07` §6.3, `M-PN1`–`M-PN12`), ainda **não versionado**, e **não** converte
