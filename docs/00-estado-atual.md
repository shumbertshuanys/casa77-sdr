...
Atualizado em: 2026-09-08 (**reconciliação documental pós-merge do resolver factual de
`caminho_yaml` — PR #135**). Esta entrega é **EXCLUSIVAMENTE DOCUMENTAL**: altera **somente**
...
dependências; **não implementa nada novo**. **A PR #133 deixou de ser a entrega funcional de
código mais recente**: o **PR #135** integrou à `main` o **resolver factual de `caminho_yaml`**
— a **linha 3 de `CY13`** —, **como fronteira isolada**. **Commit funcional
`14c95efde61c98154210fc3fc5f015e550473016`** (`feat: add caminho_yaml resolver`, **sem body, sem
trailer**), **parent `717213c69a5ebe08de330b394c4d76f72c377ee9`**, **merge commit
`4d39b821b922e19a3dd8287b5c40fe42f94a3bce`** (merge commit **normal**, dois parents:
`717213c69a…` e `14c95efde6…`; **zero squash, zero rebase, zero auto-merge**), branch de origem
`feat/caminho-yaml-resolver`. **ELA NÃO É DENOMINADA `C15`, NÃO É `E15`, NÃO É `C-A6`, NÃO CRIA
A 3B.8 e NÃO CRIA IDENTIFICADOR NORMATIVO NOVO** — é, tão somente, a **entrega funcional de
código mais recente integrada**; a **validação estrutural contextual (PR #133) passa a histórico
funcional imediatamente anterior**, com o seu registro **preservado**. **Escopo**: **dois
arquivos novos** — `src/casa77_sdr/response_yaml_resolve.py` (**+401**) e
`tests/test_response_yaml_resolve.py` (**+1551**) —, **2 arquivos, +1952 / −0**, **zero arquivo
...
`casa77_sdr.response_yaml_resolve`, com **duas** fronteiras públicas — `resolver_caminho(raiz,
decomposicao, *, item_corrente=...)` e `resolver_itera_sobre(raiz, decomposicao)` —, a exceção
pública única **`CaminhoYamlNaoResolvido`** e **`__all__` de exatamente três nomes** —
`["CaminhoYamlNaoResolvido", "resolver_caminho", "resolver_itera_sobre"]` —, **sem DTO**, **sem
dataclass**, **sem Enum** e **sem export pelo package root**. **Alcance exato — SOMENTE a linha
3 de `CY13`**: ele **recebe a estrutura factual JÁ CARREGADA EM MEMÓRIA** e a **decomposição já
produzida** pelo parser; resolve **absoluto** e **relativo**; aceita **`@` isolado**; exige
**existência de chave** e **tipo estrutural intermediário**; executa o **seletor sobre `list`**
decidindo **zero / um / múltiplos *matches***; exige que **`itera_sobre` termine em coleção**;
devolve o **terminal como está**, com **terminal `null` = resolução BEM-SUCEDIDA**; e trata
**`None` intermediário** como **falha estrutural**. **Ele NÃO abre arquivo**, **não importa
`yaml`**, **não conhece caminho de corpus**, **não rejulga a gramática** e **não muta a raiz nem
o item corrente**. **Ausência não é `None`**: a sentinela privada `_AUSENTE` distingue item
corrente **ausente** de item corrente **presente e nulo**. **Precedência materializada**: tipo
da raiz → **envelope externo** da decomposição → recusa de `(False, ())` → ausência de item
corrente (ou recusa de relativo em `itera_sobre`) → percurso, exigindo a **forma de cada
segmento somente quando ele é alcançado** — de modo que um segmento malformado **futuro** não
mascara falha anterior, enquanto a forma do segmento **corrente** vence o estado estrutural do
mesmo passo. **Falhas materializadas**: exceção pública única `CaminhoYamlNaoResolvido`,
**independente** de `CaminhoYamlInvalido` e de `CaminhoYamlContextoInvalido`, com **nove**
categorias fechadas — **`tipo_invalido`**, **`item_corrente_ausente`**,
**`relativo_em_itera_sobre`**, **`segmento_inexistente`**, **`tipo_incompativel`**,
**`travessia_em_nulo`**, **`zero_matches`**, **`multiplos_matches`** e
**`itera_sobre_nao_colecao`** — e **seis** localizadores fechados — **`raiz`**,
**`decomposicao`**, **`item_corrente`**, **`segmento`**, **`seletor`** e **`itera_sobre`** —, na
forma **`<categoria>: <localizador>`** e **sem eco do conteúdo recebido**: como este módulo toca
dado factual real, a mensagem **nunca** carrega valor recebido, valor factual, chave concreta,
literal, caminho, posição, índice, *offset*, cardinalidade, comprimento, `repr` ou tipo
concreto. **Pureza**: import único `from __future__ import annotations` — **zero `yaml`**,
**zero JSON**, **zero `re`**, **zero `unicodedata`**, **zero I/O**, **zero *filesystem***, **zero
rede**, **zero ambiente**, **zero relógio**, **zero aleatoriedade**, **zero `try`/`except`**,
**zero `raise from`**, **zero `eval`/`exec`**, **zero `assert`**, **zero estado mutável de
módulo**, **zero cópia profunda** e **zero mutação**. **ESTADO DE `CY13` APÓS ESTA
INTEGRAÇÃO**: **linha 1 — parser da gramática = MATERIALIZADA**, via
`src/casa77_sdr/response_yaml_path.py`; **linha 2 — validação estrutural contextual =
MATERIALIZADA COMO FRONTEIRA ISOLADA**, via `src/casa77_sdr/response_yaml_path_context.py`;
**linha 3 — resolver factual = MATERIALIZADA COMO FRONTEIRA ISOLADA**, via
`src/casa77_sdr/response_yaml_resolve.py`. **AS TRÊS LINHAS DE `CY13` ESTÃO MATERIALIZADAS COMO
COMPONENTES ISOLADOS — E NENHUMA DELAS ESTÁ INTEGRADA A `E1`**:
`src/casa77_sdr/response_index.py` **NÃO consome** o parser, **não consome** a validação
contextual e **não consome** o resolver; **não importa** nenhum dos três; e **continua
INALTERADO**. **NÃO se lê daqui que "CY13 está concluída em `E1`"**, nem que `E1` já aplique
qualquer regra de `caminho_yaml` além do que ela já validava antes. **A integração com `E1` é
entrega futura, ainda NÃO eleita.** **Baseline funcional corrente: `5197 passed` / Python
3.14.5**, sob **`-W error`**, com **`256 passed`** no direcionado da nova fronteira e **`603
passed`** no conjunto parser + contexto + resolver — delta **+256** sobre os **`4941 passed`**
anteriores (**4941 + 256 = 5197**), e **`347 + 256 = 603`** confirmando que as suítes do parser
e do contexto **permaneceram intactas**; **zero failures, zero errors, zero warnings** e
**nenhum teste preexistente alterado, removido ou pulado**. **A suíte nova é inteiramente
SINTÉTICA** e **nenhum teste lê o corpus versionado**. Esses números são **evidência da entrega
funcional do PR #135**, e **NÃO** uma execução desta reconciliação: **nenhum `pytest` foi
executado aqui**, porque **zero código, zero teste e zero `knowledge/**` mudaram**. **Não houve
CI configurado** para a PR #135 nem para o merge commit — *statuses* = 0, *workflow runs* = 0 —
**AUSÊNCIA DE CI/CHECKS — NÃO FALHA DE CI**. **A PR #135 NÃO**: integra a fronteira a **`E1`**;
lê arquivo; importa `yaml`; cria índice; materializa **`C-7`**; arbitra *placeholder*; arbitra
`hora`; cria *binding* físico; cria `ASSERTIVA` nova; executa a bijeção; satisfaz
**`C-A1-ST6`**–**`C-A1-ST10`**; migra **`C-11`**; nem cria `C15`, `E15`, `C-A6` ou a **3B.8**.
**ESTADO CORRENTE DE `C`**: a **gramática de `caminho_yaml` continua ARBITRADA
DOCUMENTALMENTE**; as **três linhas de `CY13` estão MATERIALIZADAS COMO FRONTEIRAS ISOLADAS E
NÃO INTEGRADAS**; o **índice físico `knowledge/indice-respostas-aprovadas.yaml` continua
INEXISTENTE**; ***placeholder* continua ABERTO**; **`hora` continua pendência normativa
separada**; **`C-7` continua NÃO MATERIALIZADA**; **`C-A1-ST6`–`C-A1-ST10` continuam NÃO
satisfeitas**; **`C-11` NÃO migrou** e a **autoridade de status continua no Markdown aprovado**;
**a 3B.8 continua INEXISTENTE**; e **`C` continua ARBITRADA / NÃO MATERIALIZADA**. **PRÓXIMA
AÇÃO**: **retornar ao GPT após a integração desta reconciliação documental para auditoria e
priorização da próxima microentrega técnica** — **nenhuma próxima microentrega é eleita aqui**,
e **não** se afirma que a próxima entrega será a **integração com `E1`**, a **materialização do
índice**, **`C-7`**, ***placeholder*** ou **`hora`**. O **item documental 110** abaixo **NÃO é
"subetapa 110"**, **NÃO é `C15`**, **NÃO é `E15`**, **NÃO é `C-A6`**, **NÃO é identificador
normativo** e **NÃO cria a 3B.8** — registra **exclusivamente** a integração funcional da
PR #135.

**Atualização anterior — 2026-09-08 (reconciliação documental pós-merge da validação estrutural
contextual de `caminho_yaml` — PR #133), preservada como registro daquele momento.** Aquela
entrega é **EXCLUSIVAMENTE DOCUMENTAL**: altera
**somente** este documento, e **não altera código, testes, `knowledge/**`,
`docs/07-arquitetura-motor-respostas.md`, `prompts/**`, `CLAUDE.md`, configuração nem
dependências; **não implementa nada novo**. **A PR #131 deixou de ser a entrega funcional de
código mais recente**: o **PR #133** integrou à `main` a **validação estrutural contextual de
`caminho_yaml`** — a **linha 2 de `CY13`** —, **como fronteira isolada**. **Commit funcional
`3097e61b61a9577c82301eaa0279f754f50c63f5`** (`feat: add caminho_yaml context validation`, **sem
body, sem trailer**), **parent `9f1dc649f84ad84175611a73af82d9aed3fb6c6b`**, **merge commit
`78158257571065efaf950b6bbaf377bb4be40d8f`** (merge commit **normal**, dois parents:
`9f1dc649f8…` e `3097e61b61…`; **zero squash, zero rebase, zero auto-merge**), branch de origem
`feat/caminho-yaml-context-validation`. **ELA NÃO É DENOMINADA `C15`, NÃO É `E15`, NÃO É `C-A6`,
NÃO CRIA A 3B.8 e NÃO CRIA IDENTIFICADOR NORMATIVO NOVO** — é, tão somente, a **entrega
funcional de código mais recente integrada**; o **parser puro da gramática (PR #131) passa a
histórico funcional imediatamente anterior**, com o seu registro **preservado**. **Escopo**:
**dois arquivos novos** — `src/casa77_sdr/response_yaml_path_context.py` (**+201**) e
`tests/test_response_yaml_path_context.py` (**+1190**) —, **2 arquivos, +1391 / −0**, **zero
arquivo preexistente alterado**, **zero `docs/**`**, **zero `knowledge/**`**, **zero
`prompts/**`**, **zero configuração**, **zero dependência**, **zero alteração de
`casa77_sdr/__init__.py`** e **zero alteração de `src/casa77_sdr/response_index.py`**.
**Contrato**: módulo `casa77_sdr.response_yaml_path_context`, com **duas** fronteiras públicas —
`validar_caminho_de_binding(caminho: str, *, fragmento_itera: bool) -> tuple[bool, tuple[tuple[str, tuple[str, str] | None], ...]]`
e
`validar_itera_sobre(caminho: str) -> tuple[bool, tuple[tuple[str, tuple[str, str] | None], ...]]` —,
a exceção pública única **`CaminhoYamlContextoInvalido`** e **`__all__` de exatamente três
nomes** — `["CaminhoYamlContextoInvalido", "validar_caminho_de_binding", "validar_itera_sobre"]`
—, **sem DTO**, **sem dataclass**, **sem Enum** e **sem export pelo package root**.
**`fragmento_itera` é keyword-only** e exigido como **`bool` exato** — `1`, `0`, `"sim"`, `[]`,
`None` e objeto com `__bool__` são **recusados**, **sem coerção de veracidade**. **Alcance exato
— SOMENTE a linha 2 de `CY13`**: **Regra A** — um `caminho_yaml` **relativo** de *binding* só é
admissível quando o fragmento **declara `itera_sobre`**, porque a raiz de um relativo é o **item
corrente**; **Regra B** — o próprio **`itera_sobre` nunca pode ser relativo** (`CY12`),
inclusive no caso de **`@` isolado** (`CY11`). **A gramática NÃO pertence a esta fronteira**: ela
continua **exclusivamente** de `analisar_caminho_yaml`, importado sob **nome privado**; o módulo
**não** procura `@`, **não** usa `startswith`, **não** relê a `str`, **não** normaliza e **não**
reimplementa a gramática — o predicado de forma relativa vem **unicamente** do resultado do
parser. **Precedência fixa**: **1.** o parser analisa o caminho; **2.** `CaminhoYamlInvalido`
**atravessa intacta** — **zero `try`**, **zero `except`**, **zero `raise from`**, **zero
*wrapping***, **zero recategorização**, com classe, mensagem, `__cause__` e `__context__`
inalterados; **3.** só então o tipo do contexto é exigido; **4.** só então a regra contextual é
aplicada — de modo que caminho gramaticalmente inválido **com** contexto inválido falha como
**`CaminhoYamlInvalido`**. **O parser é chamado UMA única vez por função**, e o sucesso devolve
**o mesmo objeto** `(relativo, segmentos)` produzido por ele — **sem reparsear**, **sem
reconstruir segmentos**, **sem `list`**, **sem DTO** e **sem `dict`**. **Falhas próprias
materializadas**: exceção pública única `CaminhoYamlContextoInvalido`, **independente** de
`CaminhoYamlInvalido` (**nem ancestral, nem descendente**; ambas derivam **diretamente** de
`Exception`), com **três** categorias fechadas — **`tipo_invalido: contexto`**,
**`relativo_sem_itera_sobre: caminho_yaml`** e **`arroba_em_itera_sobre: itera_sobre`** —, na
forma **`<categoria>: <localizador>`** e **sem eco do conteúdo recebido** (zero caminho bruto,
fragmento, tipo concreto, `repr`, posição, índice, *offset*, comprimento ou cardinalidade).
**Pureza**: **exatamente dois** imports — `from __future__ import annotations` e o parser sob
apelido privado —, **zero `re`**, **zero `unicodedata`**, **zero YAML**, **zero JSON**, **zero
I/O**, **zero *filesystem***, **zero rede**, **zero ambiente**, **zero *locale***, **zero
relógio**, **zero aleatoriedade**, **zero `try`/`except`**, **zero `raise from`**, **zero
`eval`/`exec`**, **zero `dict`/`set`** e **zero estado mutável de módulo**; a entrada **não é
alterada**. **ESTA FRONTEIRA NÃO RESOLVE E NÃO LÊ O CORPUS**: **não** abre YAML, **não** lê
`knowledge/**`, **não** verifica existência de chave, **não** executa seletor, **não** conta
*matches*, **não** confere se `itera_sobre` termina em coleção, **não** percorre o índice e
**não** substitui `validar_indice`. **ESTADO DE `CY13` APÓS ESTA INTEGRAÇÃO**: **linha 1 —
parser da gramática = MATERIALIZADA**, via `src/casa77_sdr/response_yaml_path.py`; **linha 2 —
validação estrutural contextual = MATERIALIZADA COMO FRONTEIRA ISOLADA**, via
`src/casa77_sdr/response_yaml_path_context.py` — **mas AINDA NÃO INTEGRADA A `E1`**:
`src/casa77_sdr/response_index.py` **NÃO consome** esta fronteira, **não a importa** e **continua
INALTERADO**, de modo que **não se afirma aqui que `E1` já aplique estas duas regras**; **linha 3
— resolver = NÃO MATERIALIZADA**, pois **ainda não existe** componente que percorra o YAML
factual, resolva chaves, execute seletores, valide tipo intermediário, determine **zero / um /
múltiplos *matches***, devolva terminal factual ou verifique que **`itera_sobre` termina em
coleção**. **Baseline funcional corrente: `4941 passed` / Python 3.14.5**, sob **`-W error`**,
com **`200 passed`** no direcionado da nova fronteira e **`347 passed`** no conjunto parser +
contexto — delta **+200** sobre os **`4741 passed`** anteriores (**4741 + 200 = 4941**), **zero
failures, zero errors, zero warnings** e **nenhum teste preexistente alterado, removido ou
pulado**. **A suíte nova é inteiramente SINTÉTICA** e **nenhum teste lê o corpus versionado**.
Esses números são **evidência da entrega funcional do PR #133**, e **NÃO** uma execução desta
reconciliação: **nenhum `pytest` foi executado aqui**, porque **zero código, zero teste e zero
`knowledge/**` mudaram**. **Não houve CI configurado** para a PR #133 nem para o merge commit —
*statuses* = 0, *workflow runs* = 0 — **AUSÊNCIA DE CI/CHECKS — NÃO FALHA DE CI**. **A PR #133
NÃO**: integra a fronteira a **`E1`**; implementa o resolver; lê YAML; materializa **`C-7`**;
...
**validação contextual está MATERIALIZADA COMO FRONTEIRA ISOLADA E NÃO INTEGRADA**; o
**resolver é INEXISTENTE**; o **índice físico `knowledge/indice-respostas-aprovadas.yaml`
continua INEXISTENTE**; ***placeholder* continua ABERTO**; **`hora` continua pendência normativa
separada**; **`C-7` continua NÃO MATERIALIZADA**; **`C-A1-ST6`–`C-A1-ST10` continuam NÃO
satisfeitas**; **`C-11` NÃO migrou** e a **autoridade de status continua no Markdown aprovado**;
**a 3B.8 continua INEXISTENTE**; e **`C` continua ARBITRADA / NÃO MATERIALIZADA**. **PRÓXIMA
AÇÃO**: **retornar ao GPT após a integração desta reconciliação documental para auditoria e
priorização da próxima microentrega** — **nenhuma próxima microentrega técnica é eleita aqui**,
e **não** se afirma que a próxima entrega será a **integração com `E1`**, o **resolver**,
***placeholder***, **`hora`**, **`C-7`** ou o **índice**. O **item documental 109** abaixo **NÃO
é "subetapa 109"**, **NÃO é `C15`**, **NÃO é `E15`**, **NÃO é `C-A6`**, **NÃO é identificador
normativo** e **NÃO cria a 3B.8** — registra **exclusivamente** a integração funcional da
PR #133.

**Atualização anterior — 2026-09-07 (reconciliação documental pós-merge do parser puro de
`caminho_yaml` — PR #131), preservada como registro daquele momento.** Aquela entrega é
...
**Baseline funcional corrente: `5197 passed` / Python 3.14.5.**

**Execuções da PR #135, registro do resolver factual de `caminho_yaml`**
(2026-09-08, Python 3.14.5) — **quatro execuções listadas, todas aprovadas**, medidas sobre os
**arquivos auditados no working tree**, que estavam em **CRLF** e que, **após a normalização de
fim de linha CRLF → LF realizada pelo Git**, **correspondem exatamente aos blobs integrados** —
a normalização alcança **somente o fim de linha**, **sem qualquer mudança textual, lógica ou
funcional**, e **não se afirma identidade byte-a-byte entre o working tree em CRLF e o blob em
LF**:

| Momento | Comando | Resultado |
|---|---|---|
| **preexistentes** | `python -m pytest -q -W error --ignore=tests/test_response_yaml_resolve.py` | **`4941 passed`** |
| **direcionado** | `python -m pytest -q -W error tests/test_response_yaml_resolve.py` | **`256 passed`** |
| **parser + contexto + resolver** | `python -m pytest -q -W error tests/test_response_yaml_path.py tests/test_response_yaml_path_context.py tests/test_response_yaml_resolve.py` | **`603 passed`** |
| **suíte completa estrita** | `python -m pytest -q -W error` | **`5197 passed`** |

**Zero failures, zero errors e zero warnings** sob `-W error`, com **exit code `0`** nas quatro.
O delta é **+256**, **exatamente** o número de testes coletados do arquivo direcionado **novo**
— **`4941 + 256 = 5197`** —, e **nenhum teste preexistente foi alterado, removido ou pulado**,
o que a execução de **preexistentes** confirma **ao vivo**, com os dois arquivos novos já
presentes. O conjunto **parser + contexto + resolver** confere a soma **`347 + 256 = 603`**,
evidenciando que as suítes das linhas 1 e 2 **permaneceram intactas**. Os testes foram
...
SINTÉTICA**: **nenhum teste lê o corpus versionado**. **Estes números são evidência da ENTREGA
FUNCIONAL do PR #135, e NÃO uma execução da reconciliação documental**: nesta reconciliação
**nenhum `pytest` foi executado**, porque **zero código, zero teste e zero `knowledge/**`
mudaram**. **Não há CI remoto configurado**: a PR #135 e o merge commit `4d39b821…` possuem
**zero *statuses*** e **zero *workflow runs*** — **AUSÊNCIA DE CI/CHECKS — NÃO FALHA DE CI**.
**Nenhuma execução além das reportadas é alegada.**

**Execuções anteriores — PR #133, registro da validação estrutural contextual de `caminho_yaml`**
(2026-09-08, Python 3.14.5) — **três execuções listadas, todas aprovadas**, medidas sobre os
**arquivos auditados no working tree**, que estavam em **CRLF** e que, **após a normalização de
fim de linha CRLF → LF realizada pelo Git**, **correspondem exatamente aos blobs integrados** —
a normalização alcança **somente o fim de linha**, **sem qualquer mudança textual, lógica ou
funcional**, e **não se afirma identidade byte-a-byte entre o working tree em CRLF e o blob em
LF** —, com **SHA-256 conferido antes do `git add` e novamente depois dele** e **reexecutadas
com os mesmos números imediatamente antes do merge**:

| Momento | Comando | Resultado |
|---|---|---|
| **direcionado** | `python -m pytest -W error tests/test_response_yaml_path_context.py` | **`200 passed`** |
| **parser + contexto** | `python -m pytest -W error tests/test_response_yaml_path.py tests/test_response_yaml_path_context.py` | **`347 passed`** |
| **suíte completa estrita** | `python -m pytest -W error` | **`4941 passed`** |

**Zero failures, zero errors e zero warnings** sob `-W error`, com **exit code `0`** nas três.
O delta é **+200**, **exatamente** o número de testes coletados do arquivo direcionado **novo**
— **`4741 + 200 = 4941`** —, e **nenhum teste preexistente foi alterado, removido ou pulado**;
o conjunto **parser + contexto** confere a soma **`147 + 200 = 347`**, evidenciando que a suíte
do parser **permaneceu intacta**. Os testes foram executados com o interpretador do **`.venv` do
projeto**, **Python 3.14.5** — **nenhuma alteração de ambiente, dependência ou configuração foi
feita**. **A suíte nova é inteiramente SINTÉTICA**: **nenhum teste lê o corpus versionado**.
**Estes números são evidência da ENTREGA FUNCIONAL do PR #133, e NÃO uma execução da
reconciliação documental**: nesta reconciliação **nenhum `pytest` foi executado**, porque **zero
código, zero teste e zero `knowledge/**` mudaram**. **Não há CI remoto configurado**: a PR #133 e
o merge commit `78158257…` possuem **zero *statuses*** e **zero *workflow runs*** — **AUSÊNCIA
DE CI/CHECKS — NÃO FALHA DE CI**. **Nenhuma execução além das reportadas é alegada.**

**Execuções anteriores — PR #131, registro do parser puro da gramática de `caminho_yaml`**
...
Existem agora **vinte e sete entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #135** ela é o **resolver
   factual de `caminho_yaml`**, **como fronteira isolada**, em
   `src/casa77_sdr/response_yaml_resolve.py`, registrado no **item 110** — que **não** é
   denominado `C15`, `E15` nem `C-A6`, e que **ainda NÃO está integrado a `E1`**; a **entrega
   funcional imediatamente anterior** é a **validação estrutural contextual de `caminho_yaml`**,
   em `src/casa77_sdr/response_yaml_path_context.py` (PR #133), registrada no **item 109**,
   **também não integrada a `E1`**; o **parser puro e determinístico da gramática de
   `caminho_yaml`** está em `src/casa77_sdr/response_yaml_path.py` (PR #131), registrado no
   **item 108**; a **composição total determinística de status dos fragmentos emitíveis** está
   em `src/casa77_sdr/response_status_composition.py` (PR #127), registrada no **item 106**; a
...
2. Commit funcional atual: `14c95efde61c98154210fc3fc5f015e550473016` (PR #135). Merge
   correspondente: `4d39b821b922e19a3dd8287b5c40fe42f94a3bce`. O par
   `3097e61b61a9577c82301eaa0279f754f50c63f5` / `78158257571065efaf950b6bbaf377bb4be40d8f`
   (PR #133), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`5197 passed`**, com **`256 passed`** no teste direcionado de
   `tests/test_response_yaml_resolve.py` e **`603 passed`** no conjunto parser + contexto +
   resolver, em **Python 3.14.5** — **zero failures e zero errors**, e **zero warnings** sob
   `-W error`. Baseline anterior integrado: **`4941 passed`**; delta **+256**, correspondente
   exatamente ao arquivo direcionado **novo**, **sem alteração de teste preexistente** — o que
   a execução dos **preexistentes**, com `--ignore` do arquivo novo, confirmou **ao vivo** em
   **`4941 passed`**. As execuções — **`4941`**, **`256`**, **`603`** e **`5197`**, todas sob
   `-W error` — foram **medidas na entrega funcional do PR #135, antes do merge**, sobre os
   **arquivos auditados no working tree em CRLF**, que **após a normalização de fim de linha
   CRLF → LF realizada pelo Git correspondem exatamente aos blobs integrados** — **sem mudança
   textual, lógica ou funcional**, e **sem alegação de identidade byte-a-byte** —; **nenhum
   `pytest` foi executado nesta reconciliação**. O registro anterior deste
   item — **`4941 passed`** / **`200 passed`** do PR #133 — **permanece correto como registro
   daquele momento**; ver a seção **Testes**.
...
109. **A VALIDAÇÃO ESTRUTURAL CONTEXTUAL DE `caminho_yaml` ESTÁ INTEGRADA À `main` PELO PR #133,
     COMO FRONTEIRA ISOLADA.**
     **ESTE ITEM 109 É REGISTRO DOCUMENTAL. NÃO É "SUBETAPA 109", NÃO É `C15`, NÃO É `E15`, NÃO
     É `C-A6`, NÃO É IDENTIFICADOR NORMATIVO E NÃO CRIA A 3B.8** — a **3B.8 continua
     INEXISTENTE**, e a **3B.7 continua a última subetapa numerada**. Ele registra
     **exclusivamente** a integração funcional da PR #133.
     **Integração.** **PR #133** — **commit funcional
     `3097e61b61a9577c82301eaa0279f754f50c63f5`** (`feat: add caminho_yaml context validation`,
     **sem body, sem trailer**), **parent `9f1dc649f84ad84175611a73af82d9aed3fb6c6b`**, **merge
     commit `78158257571065efaf950b6bbaf377bb4be40d8f`** (merge commit **normal**, dois parents:
     `9f1dc649f84ad84175611a73af82d9aed3fb6c6b` e
     `3097e61b61a9577c82301eaa0279f754f50c63f5`; **zero squash, zero rebase, zero auto-merge**),
     branch de origem `feat/caminho-yaml-context-validation`, **preservada**. **A `main` passou
     de `9f1dc649f84ad84175611a73af82d9aed3fb6c6b` para
     `78158257571065efaf950b6bbaf377bb4be40d8f`.**
     **Escopo.** **Dois arquivos novos** — `src/casa77_sdr/response_yaml_path_context.py`
     (**+201**) e `tests/test_response_yaml_path_context.py` (**+1190**) —, **2 arquivos,
     +1391 / −0**, **zero arquivo preexistente alterado**, **zero `docs/**`**, **zero
     `knowledge/**`**, **zero `prompts/**`**, **zero `CLAUDE.md`**, **zero configuração**, **zero
     dependência**, **zero alteração de `casa77_sdr/__init__.py`** e **zero alteração de
     `src/casa77_sdr/response_index.py`**.
     **Contrato.** Módulo `casa77_sdr.response_yaml_path_context`, com **duas** fronteiras
     públicas — `validar_caminho_de_binding(caminho, *, fragmento_itera)` e
     `validar_itera_sobre(caminho)`, ambas devolvendo
     `tuple[bool, tuple[tuple[str, tuple[str, str] | None], ...]]` —, a **exceção pública única
     `CaminhoYamlContextoInvalido`** e **`__all__` de exatamente três nomes** —
     `["CaminhoYamlContextoInvalido", "validar_caminho_de_binding", "validar_itera_sobre"]` —,
     **sem DTO**, **sem dataclass**, **sem Enum** e **sem export pelo package root**.
     **`fragmento_itera` é keyword-only** e exigido como **`bool` exato**: `1`, `0`, `"sim"`,
     `[]`, `None` e objeto com `__bool__` são **recusados** como `tipo_invalido: contexto`,
     **sem coerção de veracidade** — um contexto verdadeiro que não seja `bool` **não** libera o
     caminho relativo, e um falsy que não seja `bool` **não** substitui a ausência de
     `itera_sobre`.
     **As duas regras, e apenas duas.** **Regra A** — um `caminho_yaml` **relativo** de
     *binding* é admissível **somente** quando o fragmento **declara `itera_sobre`**, porque a
     raiz de um caminho relativo é o **item corrente da coleção percorrida** e, sem iteração,
     esse item não existe; a violação é `relativo_sem_itera_sobre: caminho_yaml`. **Regra B** —
     o próprio **`itera_sobre` nunca pode ser relativo** (**`CY12`**), inclusive no caso de
     **`@` isolado** (**`CY11`**), porque é ele que **estabelece** o item corrente e não pode
     ancorar-se num contexto que ainda não existe; a violação é
     `arroba_em_itera_sobre: itera_sobre`. **Caminho absoluto é admissível nas duas situações**:
     **iterar não proíbe endereçar a raiz do YAML**.
     **A gramática NÃO pertence a esta fronteira.** Ela continua **exclusivamente** de
     `analisar_caminho_yaml`, importado sob **nome privado**. O módulo **não** procura `@`,
     **não** usa `startswith`, **não** relê a `str`, **não** normaliza, **não** reimplementa a
     gramática e **não** faz segunda interpretação da entrada — o predicado de forma relativa é
     obtido **unicamente** do resultado do parser, e a suíte prova isso substituindo o parser por
     um espião: se o espião disser **relativo** para uma `str` **sem `@`**, a regra dispara; se
     disser **absoluto** para `"@.a"`, nada é recusado.
     **Precedência fixa.** **1.** o parser analisa o caminho; **2.** `CaminhoYamlInvalido`
     **atravessa intacta** — **zero `try`**, **zero `except`**, **zero `raise from`**, **zero
     *wrapping***, **zero recategorização**, com **classe, mensagem, `__cause__` e `__context__`
     inalterados**; **3.** só então o tipo de `fragmento_itera` é exigido; **4.** só então a
     regra contextual é aplicada. Por consequência, um caminho **gramaticalmente inválido** com
     contexto **também inválido** falha como **`CaminhoYamlInvalido`** — a precedência é do
     parser. **O parser é chamado UMA única vez por função**, tanto no sucesso quanto nas duas
     rotas de recusa, e o sucesso devolve **o mesmo objeto** `(relativo, segmentos)` — provado
     por **identidade**, não por igualdade —, **sem reparsear**, **sem reconstruir segmentos**,
     **sem `list`**, **sem DTO** e **sem `dict`**.
     **Falhas.** Exceção pública única `CaminhoYamlContextoInvalido`, **independente** de
     `CaminhoYamlInvalido` — **nem ancestral, nem descendente**, ambas derivando **diretamente**
     de `Exception` —, com **três** categorias fechadas próprias: **`tipo_invalido: contexto`**,
     **`relativo_sem_itera_sobre: caminho_yaml`** e **`arroba_em_itera_sobre: itera_sobre`**, na
     forma **`<categoria>: <localizador>`** e **sem eco do conteúdo recebido** (zero caminho
     bruto, fragmento, tipo concreto, `repr`, posição, índice, *offset*, comprimento ou
     cardinalidade).
     **Pureza.** **Exatamente dois** imports — `from __future__ import annotations` e o parser
     sob apelido privado —, **zero `re`**, `unicodedata`, YAML, JSON, `pathlib`, `os`, I/O,
     *filesystem*, rede, LLM, relógio, calendário, *locale*, variável de ambiente,
     aleatoriedade, banco, cache, logging, `try`/`except`, `raise from`, `eval`/`exec`,
     `assert`, `global`/`nonlocal`, `dict`, `set` e **estado mutável de módulo**; a entrada
     **não é alterada** e chamadas repetidas produzem **exatamente** o mesmo resultado.
     **Testes.** **`200 passed`** no direcionado, **`347 passed`** no conjunto parser + contexto
     e **`4941 passed`** na suíte completa, em **Python 3.14.5** sob **`-W error`**, com **zero
     failures, zero errors e zero warnings** e **exit code `0`** nas três. Delta **+200** sobre
     os **`4741 passed`** anteriores; a soma **`147 + 200 = 347`** evidencia que a suíte do
     parser **permaneceu intacta**. **A suíte nova é inteiramente SINTÉTICA** e **nenhum teste lê
     o corpus versionado**. **Não houve CI configurado** para a PR #133 nem para o merge commit
     — **AUSÊNCIA DE CI/CHECKS — NÃO FALHA DE CI**.
     **LIMITE DECISIVO — MATERIALIZADA MAS NÃO INTEGRADA.** A linha 2 de `CY13` está
     materializada **como componente isolado**, e **NÃO** como regra aplicada pelo motor:
     `src/casa77_sdr/response_index.py` **NÃO consome** esta fronteira, **não a importa** e
     **continua INALTERADO**; `_tem_selecao_posicional` **não foi substituída**; e **nada aqui
     autoriza ler que `E1` já valide relativo com `itera_sobre` ou já proíba `@` em
     `itera_sobre`**. **A integração com `E1` é entrega futura, ainda NÃO eleita.**
     **Esta entrega NÃO.** Ela **não** integra a fronteira a **`E1`**; **não** implementa o
     resolver; **não** lê YAML; **não** lê `knowledge/**`; **não** verifica existência de chave;
     **não** executa seletor; **não** conta *matches*; **não** confere se `itera_sobre` termina
     em coleção; **não** materializa **`C-7`**; **não** arbitra *placeholder*; **não** arbitra
     `hora`; **não** cria índice; **não** cria *binding* físico; **não** cria `ASSERTIVA` nova;
     **não** executa a bijeção; **não** satisfaz **`C-A1-ST6`**–**`C-A1-ST10`**; **não** migra
     **`C-11`**; e **não** cria `C15`, `E15`, `C-A6` ou a **3B.8**.
     **Estado de `CY13` após esta integração.** **Linha 1 — parser da gramática =
     MATERIALIZADA**, em `src/casa77_sdr/response_yaml_path.py`. **Linha 2 — validação
     estrutural contextual = MATERIALIZADA COMO FRONTEIRA ISOLADA, NÃO INTEGRADA A `E1`**, em
     `src/casa77_sdr/response_yaml_path_context.py`. **Linha 3 — resolver = NÃO MATERIALIZADA**.
     **`knowledge/indice-respostas-aprovadas.yaml` continua INEXISTENTE**, ***placeholder*
     continua ABERTO**, **`hora` continua pendência normativa separada**, **`C-7` continua NÃO
     MATERIALIZADA**, **`C-A1-ST6`–`C-A1-ST10` continuam NÃO satisfeitas**, **`C-11` NÃO
     migrou**, a **autoridade de status continua no Markdown aprovado** e **`C` continua
     ARBITRADA / NÃO MATERIALIZADA**.
     **Próxima ação.** **Retornar ao GPT após a integração desta reconciliação documental para
     auditoria e priorização da próxima microentrega.** **Nenhuma próxima microentrega técnica
     foi eleita**, e **não** se afirma que será a **integração com `E1`**, o **resolver**,
     ***placeholder***, **`hora`**, **`C-7`** ou o **índice**.
     **Relação com os itens anteriores.** **Os itens 87 a 108 permanecem corretos como registro
     do momento em que foram escritos.** Este item 109 **não os reescreve**; ele registra o
     estado **posterior**. Em particular, o **item 108** e todos os registros anteriores que
     afirmam que a **validação estrutural contextual era INEXISTENTE** — inclusive a leitura de
     **`CY13` linha 2 = NÃO MATERIALIZADA** — **permanecem corretos para o momento em que foram
     escritos** e **não foram alterados**.

110. **O RESOLVER FACTUAL DE `caminho_yaml` ESTÁ INTEGRADO À `main` PELO PR #135, COMO FRONTEIRA
     ISOLADA.**
     **ESTE ITEM 110 É REGISTRO DOCUMENTAL. NÃO É "SUBETAPA 110", NÃO É `C15`, NÃO É `E15`, NÃO
     É `C-A6`, NÃO É IDENTIFICADOR NORMATIVO E NÃO CRIA A 3B.8** — a **3B.8 continua
     INEXISTENTE**, e a **3B.7 continua a última subetapa numerada**. Ele registra
     **exclusivamente** a integração funcional da PR #135.
     **Integração.** **PR #135** — **commit funcional
     `14c95efde61c98154210fc3fc5f015e550473016`** (`feat: add caminho_yaml resolver`, **sem
     body, sem trailer**), **parent `717213c69a5ebe08de330b394c4d76f72c377ee9`**, **merge commit
     `4d39b821b922e19a3dd8287b5c40fe42f94a3bce`** (merge commit **normal**, dois parents:
     `717213c69a5ebe08de330b394c4d76f72c377ee9` e
     `14c95efde61c98154210fc3fc5f015e550473016`; **zero squash, zero rebase, zero auto-merge**),
     branch de origem `feat/caminho-yaml-resolver`, **preservada**. **A `main` passou de
     `717213c69a5ebe08de330b394c4d76f72c377ee9` para
     `4d39b821b922e19a3dd8287b5c40fe42f94a3bce`.**
     **Escopo.** **Dois arquivos novos** — `src/casa77_sdr/response_yaml_resolve.py` (**+401**)
     e `tests/test_response_yaml_resolve.py` (**+1551**) —, **2 arquivos, +1952 / −0**, **zero
     arquivo preexistente alterado**, **zero `docs/**`**, **zero `knowledge/**`**, **zero
     `prompts/**`**, **zero `CLAUDE.md`**, **zero configuração**, **zero dependência**, **zero
     alteração de `casa77_sdr/__init__.py`** e **zero alteração de
     `src/casa77_sdr/response_index.py`**. Os arquivos auditados no working tree estavam em
     **CRLF**; o Git normalizou **CRLF → LF** no *staging*, e os blobs integrados —
     `eb6efb291cbfbde2b822b877cdc52b1c13277dbf` e
     `6409db2113117ad82bb88d6594899ba66cfd4e8f` — **correspondem exatamente ao conteúdo
     auditado após essa normalização**, **sem alteração textual, lógica ou funcional**.
     **Contrato.** Módulo `casa77_sdr.response_yaml_resolve`, com **duas** fronteiras públicas —
     `resolver_caminho(raiz, decomposicao, *, item_corrente=_AUSENTE) -> object` e
     `resolver_itera_sobre(raiz, decomposicao) -> list` —, a **exceção pública única
     `CaminhoYamlNaoResolvido`** e **`__all__` de exatamente três nomes** —
     `["CaminhoYamlNaoResolvido", "resolver_caminho", "resolver_itera_sobre"]` —, **sem DTO**,
     **sem dataclass**, **sem Enum** e **sem export pelo package root**.
     **Entrada.** A **estrutura factual JÁ CARREGADA EM MEMÓRIA**, exigida como `dict`
     **exato**, e a **decomposição já produzida** pelo parser. **Quem carregou a estrutura, e se
     ela é o corpus oficial, é PRÉ-CONDIÇÃO DO CHAMADOR.** Esta é a **primeira das três
     fronteiras de `CY13` que toca dado factual** — e exatamente por isso o seu perímetro é o
     mais estreito: ela **não abre arquivo**, **não importa `yaml`**, **não conhece o caminho de
     `knowledge/casa77.yaml`**, **não descobre artefato**, **não faz I/O** e **não lê o índice**.
     **Resolução.** Percorre **absoluto** a partir da raiz e **relativo** a partir do item
     corrente; **`@` isolado** (`CY11`) devolve o **próprio item corrente por identidade** —
     `dict`, `list`, `str`, número, `bool` ou `None`; exige **existência de chave** e **`dict`
     exato** no nó intermediário; e devolve o **terminal como está, pelo próprio objeto**, sem
     cópia e sem normalização. **Terminal `null` é resolução BEM-SUCEDIDA**; **atravessar `None`
     no meio** do percurso é **falha estrutural**. **`C-7` NÃO é aplicada, reaberta nem
     materializada aqui.**
     **Seletor como OPERADOR DE CONFORMIDADE.** A coleção precisa ser `list` **exata** e é
     percorrida **inteira**, exigindo cada item `dict` exato, com a chave seletora **presente** e
     valor `str` **exato**. **A conformidade da coleção PRECEDE a cardinalidade**: um item
     malformado **vence** um *match* anterior e vence **dois**. Só depois vale **zero *matches* →
     *FAIL-CLOSED***, **um → continua**, **mais de um → *FAIL-CLOSED***. A comparação é
     **literal, por `str`, sem coerção** — o literal `"2026"` casa com a `str` e **não** com o
     inteiro `2026`, que falha por tipo.
     **`itera_sobre`.** Recusa decomposição **relativa** e exige **`list` exata** no terminal —
     mapa, escalar, `None`, `tuple` e `set` são ***FAIL-CLOSED* estrutural** (`CY12`), o que
     **não é `C-7`**. **Lista vazia é SUCESSO**, e o comportamento operacional da iteração sobre
     coleção vazia **não é decidido aqui**. A lista volta **pelo próprio objeto**, sem copiar,
     ordenar, filtrar ou materializar iterador.
     **Ausência não é `None`.** A sentinela privada `_AUSENTE` distingue item corrente
     **ausente** de item corrente **presente cujo valor é nulo**: `item_corrente=None` resolve
     `@` isolado com **SUCESSO**, enquanto a omissão levanta `item_corrente_ausente`.
     **A gramática NÃO é rejulgada.** O módulo **não** importa `analisar_caminho_yaml`, **não**
     procura `@`, **não** valida alfabeto, *whitespace*, canonicalidade ou chave exclusivamente
     numérica, **não** reconstrói o caminho e **não** concatena segmentos. Uma decomposição
     **tecnicamente bem formada** cujas `str` seriam gramaticalmente absurdas **não é recusada
     como erro gramatical**: ela é usada **estruturalmente** contra a raiz recebida.
     **Precedência fixa.** **1.** tipo da raiz; **2.** **envelope externo** da decomposição —
     `tuple` de dois, `relativo` `bool` exato, `segmentos` `tuple` exata; **3.** recusa de
     `(False, ())`, que o parser jamais produz; **4.** ausência de item corrente quando
     relativo, ou recusa de relativo em `itera_sobre`; **5.** percurso, exigindo a **forma de
     cada segmento SOMENTE quando ele é alcançado**. Um segmento malformado **futuro** **não**
     mascara falha anterior — nem a ausência do item corrente, nem chave inexistente, nem
     travessia em nulo —, enquanto a forma do segmento **corrente** vence o estado estrutural do
     **mesmo passo**. **`(True, ())` é válida**; **`(False, ())` é `tipo_invalido`**.
     **Falhas.** Exceção pública única `CaminhoYamlNaoResolvido`, **independente** de
     `CaminhoYamlInvalido` e de `CaminhoYamlContextoInvalido` — as três derivam **diretamente**
     de `Exception`, **sem parentesco entre si** —, com **nove** categorias fechadas —
     **`tipo_invalido`**, **`item_corrente_ausente`**, **`relativo_em_itera_sobre`**,
     **`segmento_inexistente`**, **`tipo_incompativel`**, **`travessia_em_nulo`**,
     **`zero_matches`**, **`multiplos_matches`** e **`itera_sobre_nao_colecao`** — e **seis**
     localizadores fechados — **`raiz`**, **`decomposicao`**, **`item_corrente`**,
     **`segmento`**, **`seletor`** e **`itera_sobre`** —, na forma
     **`<categoria>: <localizador>`**. Como este módulo **toca dado factual real**, a mensagem
     **nunca** carrega o valor recebido, o valor factual, a chave concreta, o literal, o
     caminho, um fragmento dele, uma posição, um índice, um *offset*, uma cardinalidade
     observada, um comprimento, o `repr` ou o tipo concreto — **em todas as nove categorias**.
     **Tipos exatos.** `type(x) is dict` e `type(x) is list` para a estrutura YAML; `tuple`,
     `bool` e `str` exatos para os componentes da decomposição. **Subclasse de `dict` ou de
     `list` é recusada** na raiz, no nó intermediário, no item corrente, na coleção do seletor,
     no item da coleção e no terminal de `itera_sobre` — porque uma subclasse pode redefinir
     `__contains__`, `__getitem__`, `__eq__` ou `__iter__` e decidir por conta própria o que a
     estrutura diz.
     **Pureza.** Import único `from __future__ import annotations` — **zero `yaml`**, JSON,
     `re`, `unicodedata`, `pathlib`, `os`, `open`, I/O, *filesystem*, rede, LLM, relógio,
     calendário, *locale*, variável de ambiente, aleatoriedade, banco, cache, logging,
     `try`/`except`, `raise from`, `eval`/`exec`, `assert`, `global`/`nonlocal` e **estado
     mutável de módulo**; a **raiz não é mutada**, o **item corrente não é mutado** e **nada é
     copiado em profundidade**.
     **Testes.** **`256 passed`** no direcionado, **`603 passed`** no conjunto parser + contexto
     + resolver, **`4941 passed`** nos preexistentes com `--ignore` do arquivo novo e **`5197
     passed`** na suíte completa, em **Python 3.14.5** sob **`-W error`**, com **zero failures,
     zero errors e zero warnings** e **exit code `0`** nas quatro. Delta **+256** sobre os
     **`4941 passed`** anteriores; as somas **`347 + 256 = 603`** e **`4941 + 256 = 5197`**
     evidenciam que as suítes das linhas 1 e 2 **permaneceram intactas**. **A suíte nova é
     inteiramente SINTÉTICA** e **nenhum teste lê o corpus versionado**. **Não houve CI
     configurado** para a PR #135 nem para o merge commit — **AUSÊNCIA DE CI/CHECKS — NÃO FALHA
     DE CI**.
     **LIMITE DECISIVO — AS TRÊS LINHAS MATERIALIZADAS, NENHUMA INTEGRADA.** Com esta entrega as
     **três linhas de `CY13` passam a existir em código**, mas **como componentes isolados**, e
     **NÃO** como regras aplicadas pelo motor: `src/casa77_sdr/response_index.py` **NÃO consome**
     o parser, **NÃO consome** a validação contextual e **NÃO consome** o resolver; **não importa
     nenhum dos três**; `_tem_selecao_posicional` **não foi substituída**; e **continua
     INALTERADO**. **NÃO se lê daqui que "`CY13` está concluída em `E1`"**, nem que `E1` já
     aplique qualquer regra de `caminho_yaml` além do que já validava antes. **A integração com
     `E1` é entrega futura, ainda NÃO eleita.**
     **Esta entrega NÃO.** Ela **não** integra a fronteira a **`E1`**; **não** lê arquivo;
     **não** importa `yaml`; **não** cria o índice; **não** materializa **`C-7`**; **não**
     arbitra *placeholder*; **não** arbitra `hora`; **não** formata valor; **não** cria *binding*
     físico; **não** cria `ASSERTIVA` nova; **não** executa a bijeção; **não** satisfaz
     **`C-A1-ST6`**–**`C-A1-ST10`**; **não** migra **`C-11`**; e **não** cria `C15`, `E15`,
     `C-A6` ou a **3B.8**.
     **Estado de `CY13` após esta integração.** **Linha 1 — parser da gramática =
     MATERIALIZADA**, em `src/casa77_sdr/response_yaml_path.py`. **Linha 2 — validação
     estrutural contextual = MATERIALIZADA COMO FRONTEIRA ISOLADA, NÃO INTEGRADA A `E1`**, em
     `src/casa77_sdr/response_yaml_path_context.py`. **Linha 3 — resolver factual =
     MATERIALIZADA COMO FRONTEIRA ISOLADA, NÃO INTEGRADA A `E1`**, em
     `src/casa77_sdr/response_yaml_resolve.py`.
     **`knowledge/indice-respostas-aprovadas.yaml` continua INEXISTENTE**, ***placeholder*
     continua ABERTO**, **`hora` continua pendência normativa separada**, **`C-7` continua NÃO
     MATERIALIZADA**, **`C-A1-ST6`–`C-A1-ST10` continuam NÃO satisfeitas**, **`C-11` NÃO
     migrou**, a **autoridade de status continua no Markdown aprovado** e **`C` continua
     ARBITRADA / NÃO MATERIALIZADA**.
     **Próxima ação.** **Retornar ao GPT após a integração desta reconciliação documental para
     auditoria e priorização da próxima microentrega técnica.** **Nenhuma próxima microentrega
     foi eleita**, e **não** se afirma que será a **integração com `E1`**, a **materialização do
     índice**, **`C-7`**, ***placeholder*** ou **`hora`**.
     **Relação com os itens anteriores.** **Os itens 87 a 109 permanecem corretos como registro
     do momento em que foram escritos.** Este item 110 **não os reescreve**; ele registra o
     estado **posterior**. Em particular, o **item 109** e todos os registros anteriores que
     afirmam que o **resolver era INEXISTENTE** — inclusive a leitura de **`CY13` linha 3 = NÃO
     MATERIALIZADA** e as baselines de **`4941 passed`** e anteriores — **permanecem corretos
     para o momento em que foram escritos** e **não foram alterados**.
