...
Existem agora **vinte e quatro entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
1. A **sétima microentrega funcional de `C`** — o **canonicalizador determinístico de rótulo
   ...
   funcional mais recente NÃO é mais a do PR #97**: desde o **PR #127** ela é a **composição
   total determinística de status dos fragmentos emitíveis**, em
   `src/casa77_sdr/response_status_composition.py`, registrada no **item 106** — que **não** é
   denominada `C15`; a **entrega funcional imediatamente anterior** é a **associação física
   determinística de seção e fragmentos**, em
...
2. Commit funcional atual: `24d2b0fef6a08c26e751f8eb8bb70943b30c0339` (PR #127). Merge
   correspondente: `00c45e9b95f233bc7aa0f9666e090208994fb190`. O par
   `0e41165406c3a9261a1fcc8d0f9c600b0469e730` / `ecaf0a2b3a594f8fedd10f708461ad2eeb18e086`
   (PR #124), registrado aqui anteriormente, **permanece correto como registro daquele
   momento**.
3. Baseline funcional atual: **`4594 passed`**, com **`117 passed`** no teste direcionado de
   `tests/test_response_status_composition.py`, em **Python 3.14.5** — **zero failures e zero
   errors**, e **zero warnings** sob `-W error`. Baseline anterior integrado: **`4477
   passed`**; delta **+117**, correspondente exatamente ao arquivo direcionado **novo**,
   **sem alteração de teste preexistente**. As execuções — **`117`**, **`134`** (associação
   física), **`243`** (C14), **`397`** (C13), **`299`** (C12), **`201`** (C8) e **`4594`**,
   todas sob `-W error` — foram **medidas na entrega funcional do PR #127, antes do merge**,
   sobre os bytes que vieram a ser exatamente os blobs integrados; **nenhum `pytest` foi
   executado nesta reconciliação**. O registro anterior deste item — **`4477 passed`** /
   **`134 passed`** do PR #124 — **permanece correto como registro daquele momento**; ver a
   seção **Testes**.
