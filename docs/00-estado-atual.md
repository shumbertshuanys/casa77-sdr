...
Atualizado em: 2026-09-01 (**reconciliação documental após o merge do PR #91**). A **PR #91**
...
| Base da reconciliação **pós-PR #91** | `d15201b0a84bca332b09e0d5e623736605663962` — HEAD da `main` verificado **antes** desta reconciliação |
...
**Baseline funcional corrente: `1908 passed` / Python 3.14.5.**

**Execuções auditadas da PR #91, registro desta entrega** (2026-09-01, Python 3.14.5) —
...
**Esta reconciliação NÃO reexecutou a suíte**, por ser **exclusivamente documental**: as
contagens acima são as **auditadas pela PR #91**. A árvore de trabalho testada estava em
...
**Baseline funcional atual: `1908 passed`.** Baseline anterior integrado: **`1589 passed`**
— delta **+319**, correspondente **exatamente** ao arquivo direcionado **novo**
`tests/test_response_format.py`; **nenhum teste preexistente foi alterado** pela PR #91. O
...
Existem agora **doze entregas funcionais posteriores à 3B.7 e SEM numeração oficial de
...
pura de `C-6`** em `src/casa77_sdr/response_format.py` —, integrada pelo **PR #91**.
**Nenhuma das quatro materializa `C`**: o índice `knowledge/indice-respostas-aprovadas.yaml`
**continua inexistente**, o carregador **não conhece caminho implícito** para ele, o
comparador **opera sobre `str` que lhe são entregues**, sem analisar Markdown e sem I/O, e os
formatadores **recebem valores já resolvidos**, sem consultar fonte alguma e **sem consumidor
integrado**. O formato **`hora` continua NÃO MATERIALIZADO**.
...
1. A **quarta microentrega funcional de `C`** — os **formatadores determinísticos de
   apresentação pura de `C-6`**, em `src/casa77_sdr/response_format.py` — está
   **funcionalmente concluída e integrada à `main`** pelo **PR #91** (**MERGED**). Ela
   **não recebeu numeração de subetapa** e **não criou nomenclatura normativa `E2`, `E3` ou
   `E4`**. A **entrega funcional anterior** é a **terceira microentrega — o comparador
   determinístico de equivalência textual de `C-15b`** (PR #89), que permanece integrada.
   **A entrega funcional mais recente é a do PR #91.** **Nenhuma das quatro microentregas
   materializa `C`**: o índice `knowledge/indice-respostas-aprovadas.yaml` **continua
   INEXISTENTE** e **`C` continua ARBITRADA / NÃO MATERIALIZADA como entrega completa**.
   **Formatar não é materializar**: os formatadores **recebem valores já resolvidos**,
   devolvem **apresentação pura** e **não leem fonte alguma, não fazem I/O, não consultam
   *locale*, não conhecem o índice, o Markdown, o *template*, o *renderer* nem qualquer
   consumidor**. Foram materializados **cinco** formatos — **`inteiro`**,
   **`inteiro_agrupado`**, **`simbolo_moeda`**, **`texto`** e **`lista`**; o formato
   **`hora` continua NÃO MATERIALIZADO**, por **lacuna normativa ainda não arbitrada** sobre
   ...
   arbitra**.
2. Commit funcional atual: `7d8dd8617eb5cd8c346e67496c3631feafe97f4f`. Merge
   correspondente: `d15201b0a84bca332b09e0d5e623736605663962`.
3. Baseline funcional atual: **`1908 passed`**, com **`319 passed`** no teste direcionado de
   `tests/test_response_format.py`, em **Python 3.14.5** — **zero failures e zero errors**,
   e **zero warnings** sob `-W error`. Baseline anterior integrado: **`1589 passed`**; delta
   **+319**, correspondente exatamente ao arquivo direcionado **novo**, **sem alteração de
   teste preexistente**. A **PR #91 auditou as quatro execuções** — **`319`** e **`1908`**,
   ambas também sob `-W error` —, sobre bytes cuja **invariância foi provada por SHA-256
   antes e depois**; **esta reconciliação não as reexecutou** — ver a seção **Testes**.
