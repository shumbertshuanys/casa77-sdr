...
| Fronteira | Devolve |
|---|---|
| **S2-D8** / `R2` | os fragmentos que **cobrem a consulta** (`SF-D4-10`) — o **produtor determinístico está materializado** (§4.4.3) e a **infraestrutura estrutural de `R2`** também (§4.4.2); o **conteúdo aprovado do mapa** está **materializado** em `knowledge/mapa-cobertura.yaml`, com a **associação total dos assuntos**; a **aplicabilidade determinística de pacote** está **materializada** (§4.4.4) |
...
| PE-2 | **Duas entradas base, e uma terceira condicional.** As obrigatórias são `fragmentos_autorizados` — a tupla de **S2-D8** (`SF-D4-10`) — e `acoes` — as ações da **primeira** decisão da máquina; nenhuma das duas é reavaliada aqui. A terceira é **`fotografia_r06`**, ***keyword-only*** e **condicional**: ela é exigida **somente** quando a ação de **T16** é a ***owner*** de `R06`/`F1` — ação presente **e** cobertura que **não** trouxe o token (**PE-15**). Fora desse caminho ela é **irrelevante**: não é lida, não é validada e não é avaliada. Chamada com **dois** argumentos continua válida. |
...
| PE-6 | **Três unidades materializadas nesta versão, com naturezas distintas.** **`R03`/`F1`** é **mandatório puro**: só chega pela **ação de lacuna**, nunca por cobertura. **`R05`/`F1`** é **dupla rota sem *bindings*** para **T15** (**PE-13**): a cobertura prevalece quando já o trouxe, e a ação o completa quando não trouxe. **`R06`/`F1`** é **dupla rota condicionada à emissibilidade** para **T16** (**PE-15**–**PE-20**): ele **tem *bindings*** — de origem `YAML`, nunca `RUNTIME_AUTORITATIVO` —, e por isso a **rota da ação não o acrescenta sem veredito**. Os **dois primeiros** são admissíveis porque o ***gate* mecânico do corpus versionado** prova que existem, que o rótulo canônico os admite e que têm **zero *bindings*** — **D8-F2** e **D8-F3** satisfeitas **vacuamente**. O **terceiro** é admissível por caminho **diferente**: a emissibilidade é **decidida em execução**, pela **autoridade única** de §4.4.5. **Isto não afirma que S2-D8 reavalie esses fragmentos**, e **não abre genericamente** mandatórios com *binding*: qualquer mapeamento futuro **exige nova arbitragem**. |
| PE-7 | **Zero contribuição significa UMA coisa só**: *esta fronteira não acrescenta fragmento aprovado para aquela ação naquele ciclo*. Ela **não** afirma que a ação foi atendida, que ela não é textual, que foi dispensada ou que a obrigação conversacional desapareceu. Vale para a **tupla vazia** na tabela e, igualmente, para **`R06`/`F1` não emitível** pela rota da ação (**PE-18**): a obrigação conversacional **permanece**, agora **degradada**, e continua pertencendo à futura política da etapa 10 — o projetor **não inventa conteúdo**, **não escolhe substituto** e **não produz evento** para resolvê-la. |
| PE-8 | **Ordem: recebidos → mandatórios.** Primeiro a tupla de cobertura, na ordem recebida; depois os mandatórios, na **ordem das ações**. **Zero ordenação lexical.** No escopo atual isso põe o fragmento de lacuna **ao final** sempre que houver cobertura (**PC-4**). Quando a **cobertura é a *owner*** — de `R05`/`F1` (**PE-13**) ou de `R06`/`F1` (**PE-16**) —, o token **não** é mandatório naquele ciclo e **mantém a posição original** dentro de `fragmentos_autorizados`: ele **não** migra para o bloco mandatório nem para o fim. |
...
| PE-10 | **Colisão entre as duas origens é *fail-closed***, **exceto** pelas **duas** duplas rotas explicitamente fechadas: **PE-13**, de `R05`/`F1`, e **PE-15**–**PE-20**, de `R06`/`F1`. Fora delas, um identificador que chegue **pelas duas** significa modelagem incoerente a montante — e isso continua protegendo contra modelar o fragmento de lacuna como alternativa de cobertura: **`R03`/`F1`** por cobertura **mais** a ação de lacuna **fecha**, como antes. **Não existe deduplicação *cross-source* genérica**, duplicatas **não** passaram a ser aceitas, e **nenhum token futuro herda a exceção por analogia**: são **dois *owners* explícitos**, cada um com **regra própria** — `R06` **não** reutiliza o mecanismo de `R05` —, e **não** um mecanismo geral. |
| PE-11 | ***Gate* mecânico de corpus, em duas classes.** Todo identificador não vazio da tabela é conferido **nos testes** contra o índice físico: existe no domínio canônico e tem **rótulo canônico emitível**. A partir daí as classes divergem. **Classe estática** — **`R03`/`F1`** e **`R05`/`F1`** — continua exigindo **zero *bindings***. **Classe condicionada** — **`R06`/`F1`** — exige o oposto e mais uma garantia: ele **possui *bindings*** e **nenhum** deles é de origem **`RUNTIME_AUTORITATIVO`**, de modo que **nenhum fato de runtime** entra nesta fronteira. **Nenhuma cardinalidade fixa de *bindings*** é exigida como regra futura, e **nenhum conteúdo ou valor comercial é copiado**. Se qualquer uma dessas pré-condições deixar de valer, o teste fica **vermelho de propósito**. Em **produção**, nada disso é lido. |
| PE-12 | **Zero decisão comercial e zero texto.** A fronteira não escolhe o que cobre a consulta, não avalia regra comercial, não resolve *binding*, não monta texto, não chama LLM e **não executa ação alguma**. Ela projeta **identificadores**. Ela também **não implementa emissibilidade**: para o caso fechado de **`R06`/`F1`** ela **consome exclusivamente** a **autoridade única** `avaliar_emissibilidade` (§4.4.5), sobre uma **fotografia já montada e recebida**. **Nenhuma segunda implementação de `D8-F` é criada**, e a fronteira continua **sem** comparar status, percorrer referentes, avaliar `ASSERTIVA`, resolver YAML, ler índice ou montar fotografia. |
| PE-13 | ***Owner* de `R05`/`F1` — dupla rota fechada.** `R05`/`F1` satisfaz a mesma obrigação textual de **T15** por **duas** rotas: a **cobertura** de S2-D8 e a ação **`INFORMAR_NAO_CONFIRMACAO_DE_DISPONIBILIDADE`**. A regra é literal: **cobertura presente → a cobertura é a *owner*** e o bloco mandatório **não acrescenta** segunda ocorrência; **cobertura ausente → a ação é a *owner*** e acrescenta o token. O resultado tem **exatamente uma** ocorrência, **sem alteração de ordem** (**PE-4**, **PE-8**). A exceção vale **somente** para este par ação/token. Ela **não** faz *lookup* em produção (**PE-1**), **não** decide candidatura, **não** reavalia **`D8P-11`**, **não** altera **S2-D8** e **não** altera **CAL6**. |
| PE-14 | **Variantes incompatíveis da superfície R05.** Com a ação de **T15** presente, **`R05`/`F2`** ou **`R05`/`F3`** em `fragmentos_autorizados` é combinação **incoerente para emissão**: as duas variantes sustentam uma resposta de disponibilidade a partir de **consulta autoritativa válida**, e T15 ordena o ***fallback* de não confirmação**. Resultado: ***fail-closed***. O projetor **não escolhe `F1`**, **não escolhe `F2`/`F3`**, **não corrige a condição 6**, **não altera fatos de runtime** e **não altera S2-D8** — ele apenas **rejeita insumos contraditórios**. **Sem a ação de T15**, as duas variantes atravessam normalmente: a incompatibilidade é **com a ação**, não uma proibição do token. |
| PE-15 | ***Owner* de `R06`/`F1` — dupla rota condicionada, com regra própria.** `R06`/`F1` satisfaz a obrigação textual de **T16** por **duas** rotas: a **cobertura** de S2-D8 e a ação **`INFORMAR_CONDICOES_DE_VISITA`**. **Nada aqui é herdado de `PE-13` por analogia**: a diferença material é que `R06`/`F1` **tem *bindings***, e por isso a rota da ação **depende de veredito**. A regra é literal: **cobertura presente → a cobertura é a *owner***; **cobertura ausente → a ação é a *owner***, e ela só acrescenta o token **se a emissibilidade for afirmada**. |
| PE-16 | **Cobertura *owner*.** Quando `R06`/`F1` já consta de `fragmentos_autorizados` e a ação de T16 está presente, o token **mantém a posição original** (**PE-4**, **PE-8**), o bloco mandatório **não acrescenta** segunda ocorrência, e o resultado tem **exatamente uma**. Nesse caminho a `fotografia_r06` é **completamente irrelevante**: ela **não é lida**, **não é validada**, **não é avaliada** e a **autoridade única não é consultada** — nem mesmo se um valor de tipo inválido tiver sido passado. |
| PE-17 | **Ação *owner*, fragmento emitível.** Sem o token na cobertura, a ação consulta **`avaliar_emissibilidade`** (§4.4.5) sobre a `FotografiaFragmento` recebida. Com **`emitivel is True`**, `R06`/`F1` entra no **bloco mandatório**, preservando **recebidos primeiro, mandatórios na ordem das ações**. A avaliação ocorre **no máximo uma vez por chamada** de `projetar_fragmentos_para_emissao`, e **somente quando efetivamente necessária**: o veredito é memorizado **na própria chamada**, com **zero *cache* global**. Ação repetida **não duplica** o token nem a consulta. |
| PE-18 | **Ação *owner*, fragmento não emitível.** Com **`emitivel is False`**, o resultado é **zero fragmento acrescentado por T16**, sob a leitura literal de **PE-7**. Isso **não é erro**: nenhuma exceção é levantada, **nenhum resultado parcial** é devolvido, **nenhum quarto status** é criado, **nenhum substituto** é escolhido — nem `R03`/`F1`, nem `R05`/`F1` — e **nenhum texto é inventado**. A obrigação conversacional **degradada permanece residual** da futura política da etapa 10. |
| PE-19 | ***Fail-closed* da fotografia, e só na rota da ação.** Quando a ação de T16 é a *owner*, `fotografia_r06` **ausente** ou de tipo diferente de **`FotografiaFragmento`** exato fecha com **`tipo_invalido: fotografia_r06`**, pela exceção pública **já existente** — **nenhuma categoria nova** e **nenhuma exceção pública nova** são criadas. Subclasse **não** é forma canônica. Fora desse caminho — cobertura *owner*, ou ação de T16 ausente — a entrada **não é validada**. |
| PE-20 | **Limites de `R06`.** Esta rota **não produz evento**: **zero `E09`**, **zero `E18`**, **zero `CausaE09`**, **zero `MotivoE09`**, **zero `MotivoHandoff`**. Ela **não exige nova chamada da `MaquinaEstados`**, **não cria efeito paralelo** e **não dispara *handoff***. **T16 na máquina permanece literal** — `E10` → `T16` → `INFORMAR_CONDICOES_DE_VISITA` —, e **interesse simples em visita continua sendo T16**: pedido de **confirmação** de visita permanece matéria distinta e **humana**. `state_machine.py`, `cycle_events.py`, `cycle_inputs.py`, `handoff_detection.py`, `coverage_decision.py` e `fragment_emissibility.py` **não são tocados**, e **nenhuma ação, condição, motivo ou componente novo** é criado. |
...
*end-to-end*** da etapa 10; as **superfícies conversacionais residuais**, ainda
sem fragmento aprovado; e o **`OrquestradorMotor`**. A **cadeia de `R2` não está entre elas**:
a **infraestrutura estrutural de `R2`** está **materializada** (§4.4.2), o **produtor
determinístico de S2-D8** está **materializado** (§4.4.3) e o **conteúdo aprovado do mapa**
está **materializado** em `knowledge/mapa-cobertura.yaml`. **§4.2 é reconciliada somente no
ponto que descreve o `ValidadorResposta`; as demais matérias de §4.2 permanecem vigentes.**
...
#### 4.1.10 Contrato vivo do `OrquestradorMotor` — recorte `R1`

O contrato de implementação deste recorte vive em `src/casa77_sdr/orchestrator.py`, com os
cenários em `tests/test_orchestrator.py`. Ele materializa o **primeiro prefixo executável e
permanente** do componente `OrquestradorMotor` de §4.1 — as etapas **1**, **2** e **3** do
pipeline (§5) e o **tratamento operacional TS45** (§7.1, `TS45-1`–`TS45-20`) — e **nada além
disso**.

**R1 não é componente novo.** §4.1 permanece com **14** componentes, §2 com **nove**
responsabilidades, o pipeline de §5 com **catorze** etapas e os encerramentos sem transição
com **quatro** casos (doc 06 §4.5). R1 materializa **parte** de um componente que já existia
na tabela: ele **não é um 15º componente** e **não é etapa nem subetapa nova**.

**R1 é runtime PARCIAL, não integração completa.** Ele **não** liga o ciclo, **não** libera o
primeiro teste conversacional local e **não** reabre N-a, N-b, AJ1–AJ4, S2-D8, S3-D1, E4, a
qualificação, a identidade nem a `MaquinaEstados`. **Nenhuma fronteira existente foi alterada
por ele**: `normalization.py`, `context.py`, `eligibility.py` e `persistence.py` permanecem
intactos, e `__init__.py` também.

| # | Contrato de implementação OMR1 |
|---|---|
| OMR1-1 | **Recorte e posição.** `R1` cobre **exatamente** a **etapa 1** (normalização e chave, §4.3), a **etapa 2** (decisão de idempotência) e a **etapa 3** (recuperação de contexto e projeções de identidade, §6.2), mais o **tratamento operacional TS45** quando a etapa 3 bloqueia. **Não cobre** interpretação (4), identidade (5), atualização de dados (6), qualificação (7), projeção de insumos (8), seleção de fatos (9), composição (10), validação (11), fechamento determinístico (12), persistência (13) e emissão (14). |
| OMR1-2 | **Superfície pública.** **Exatamente um** nome — a função `coordenar_etapas_1_a_3(entrada, *, persistencia, janela_idempotencia, limiar_recencia, tentar_alerta)` —, declarado em `__all__` e **não exportado** pelo `__init__.py` do pacote. Helpers privados no mesmo módulo são permitidos; **nenhum** deles é superfície. |
| OMR1-3 | **Zero tipo novo.** **Nenhuma** classe `OrquestradorMotor`, `ResultadoOrquestrador`, DTO, `enum`, exceção pública, `Protocol` ou interface de alerta é criada. As saídas são os tipos **já existentes** `EntradaNormalizada` (§4.3) e `ProjecoesIdentidadeEtapa3` (§6.2); a entrada é a `EntradaMensagem` do contrato comum (§6.1). |
| OMR1-4 | **Janela e limiar explícitos — zero default.** `janela_idempotencia` e `limiar_recencia` chegam **do chamador**, **sem valor padrão**, e o módulo **não** os lê de variável de ambiente, arquivo de configuração, constante ou literal temporal. A **janela** é validada **exclusivamente** por `normalizar_entrada(...)` (§4.3); o **limiar** é **apenas transportado** para `montar_projecoes_identidade_etapa3(...)` e **não é revalidado** aqui — é o que faz `ConfiguracaoTemporalInvalida` nascer da autoridade já existente (`exigir_limiar_valido`, passo 1 de §6.2) e cair no gatilho de TS45 sob **S10**. |
| OMR1-5 | **Alerta injetado — dependência obrigatória.** `tentar_alerta` é **parâmetro obrigatório e sem default**. **Passo 0**, antes de qualquer etapa e **antes de tocar a persistência**: se não for chamável, o recorte levanta **`TypeError`** com mensagem **sanitizada**, que **não reproduz o valor recebido** (§6.6). R1 **não escolhe** destino, canal, provedor, transporte, SDK, formato, SLA nem confirmação de entrega (`TS45-15`). |
| OMR1-6 | **Payload do alerta — fechado em três informações.** A chamada é **obrigatoriamente por palavras-chave**: `categoria`, `pendente_preservado` e `correlacao`. `categoria` é `type(erro).__name__` do bloqueio, sempre uma das **sete** classes autorizadas; `pendente_preservado` é `True`/`False`; `correlacao` é a `EntradaNormalizada.chave_idempotencia`, **opaca por construção**. **Proibidos**: mensagem, canal, contato, `id_atendimento`, dado comercial, `str(exc)`, `repr(exc)`, *stack trace*, segredo e credencial (`TS45-14`). **Sem DTO** e **sem `Protocol`**; o **retorno é ignorado**, e sucesso é a **ausência de exceção**. |
| OMR1-7 | **Sequência.** **0.** validar o contrato da dependência de alerta; **1.** `normalizar_entrada(entrada, janela_idempotencia)`; **2.** `persistencia.chave_processada(chave)`; **3.** `montar_projecoes_identidade_etapa3(...)` com `canal`, `contato`, `id_atendimento_informado`, `instante_de_referencia_do_ciclo` e `limiar_recencia`. Nenhum passo é antecipado, reordenado ou duplicado. |
| OMR1-8 | **Zero relógio vivo.** O `instante_de_referencia_do_ciclo` é o campo "data e hora" da própria entrada (§6.1, N-a-R2). O módulo **não** consulta `now`, `today`, `utcnow` nem qualquer fonte de tempo corrente, e **não** constrói duração literal. |
| OMR1-9 | **Caminho feliz.** Concluída a etapa 3, o recorte devolve a tupla `(EntradaNormalizada, ProjecoesIdentidadeEtapa3)` e **nada mais**: **não** marca a chave, **não** persiste estado, **não** interpreta, **não** resolve identidade, **não** qualifica, **não** chama a `MaquinaEstados` e **não** emite. A marcação da chave do **ciclo normal** pertence às etapas posteriores, quando o ciclo completo existir. |
| OMR1-10 | **Terminais `None`.** Os **três** desfechos terminais do recorte devolvem `None`: **mensagem vazia** na etapa 1 — **zero efeito**, sem tocar a persistência —, **chave já processada** na etapa 2 — **zero** pendente, **zero** alerta, **zero** marcação nova e **zero** etapa 3 (`TS45-9`, `E4-9`) — e **bloqueio TS45** concluído. Eles são **estados distintos** e permanecem distinguíveis pelos **efeitos observáveis** na persistência: R1 **não cria** código de retorno, enum ou DTO para diferenciá-los. |
| OMR1-11 | **Captura TS45 fechada.** A etapa 3 é envolvida por **uma única** captura, cuja tupla contém **exatamente as sete classes nomeadas** de `TS45-2`: `ConfiguracaoTemporalInvalida`, `IdentificadorNaoResolvido`, `ContextoElegibilidadeCorrompido`, `MarcoTemporalAusente`, `IdentificadoIncoerente`, `ConjuntoHumanoIncoerente` e `ProjecaoIdentificadorIncoerente`. **`except ValueError:` é proibido**, **`except Exception:` é proibido** como detecção de TS45 e **`BaseException` nunca é capturada**. Qualquer exceção fora das sete — inclusive uma futura falha de leitura da persistência (`TS45-19`) — **propaga intacta**, sem pendente, sem alerta e sem marcação. `EntradaInvalida` (§6.1) também **propaga intacta** e **não** é TS45. |
| OMR1-12 | **Efeitos TS45 e ordem.** Sob qualquer das sete: **1.** preservar `ProcessamentoPendente(canal, contato, conteudo)` com os **três campos existentes** e `conteudo` = **`EntradaNormalizada.mensagem_normalizada`**, nunca a mensagem bruta (`TS45-4`, `TS45-5`); **2.** **tentar** o alerta; **3.** marcar a chave por `marcar_chave_processada`, **fora da etapa 13** e **somente após preservação bem-sucedida** (`TS45-8`); **4.** encerrar **sem transição**. Preservadas as negações de `TS45-17`: zero máquina, zero evento, zero transição, zero emissão, zero handoff, zero atendimento criado, zero etapa 5, zero etapa 13, zero `CondicoesCiclo`, zero S2-D8 e `instante_ultima_transicao` **não atualizado**. A etapa 4 **não executa** sob bloqueio (`TS45-18`). |
| OMR1-13 | **Semântica e precedência de falhas.** **(A)** Preservação bem-sucedida e **alerta falhando**: a exceção do alerta é **capturada e absorvida**, a chave **é marcada** e o recorte devolve `None` (`TS45-12`) — **zero** retry, fila, contador ou status de entrega. **(B)** **Preservação falhando**: o alerta ainda é **tentado**, com `pendente_preservado=False` (`TS45-11`); uma falha **do alerta não substitui** a falha da preservação; a chave **não é marcada**; e a **exceção da preservação** é relançada **por identidade**. A exceção TS45 que originou o tratamento **não** é a que propaga. **(C)** **Marcação falhando**, com pendente já preservado e alerta já tentado: a **exceção da marcação** propaga **por identidade** (`TS45-13`) — **sem** retry, **sem** compensação e **sem** apagar o pendente. |
| OMR1-14 | **Fora do escopo e pureza.** O módulo **não importa** e **não chama** `interpretation_stage`, `interpretation`, `identity.resolver_identidade`, `data_update`, `rules`, `qualification`, `coverage_decision`, `cycle_events`, `handoff_detection`, `closure_decision`, `cycle_inputs`, `state_machine.decidir`, `fragment_snapshot`, `emission_projection`, `criar`/`gravar` da persistência nem qualquer escrita de marco temporal. **Zero** LLM, YAML, `knowledge/**`, calendário, rede, SDK, *logging*, filesystem, cache, retry, *sleep* e aleatoriedade. Existe **exatamente um** `except Exception:` no módulo, e ele envolve **somente** a chamada de `tentar_alerta(...)`. |

**Cobertura dos sete gatilhos.** Seis das sete classes são exercitadas **em runtime** através
de `coordenar_etapas_1_a_3(...)`, com entradas conformes e implementações legítimas de
`PersistenciaOperacional` — a volátil `PersistenciaEmMemoria` e *doubles* controlados do
mesmo padrão já usado em `tests/test_context.py`: `ConfiguracaoTemporalInvalida`,
`IdentificadorNaoResolvido`, `ContextoElegibilidadeCorrompido`, `MarcoTemporalAusente`,
`IdentificadoIncoerente` e `ConjuntoHumanoIncoerente`.
**`ProjecaoIdentificadorIncoerente` permanece defesa estrutural**: sob a superfície pública
da etapa 3, cada uma das suas condições é **inalcançável** sem violar contrato a montante —
`NAO_ENCONTRADO`/`INCOMPATIVEL` já bloqueiam antes, em `IdentificadorNaoResolvido`;
`havia_estado_esperado` é **verdadeiro por construção** sob `ENCONTRADO`; e a unicidade da
ocorrência já é imposta antes, por `IdentificadoIncoerente`. **Nenhuma persistência inválida
é fabricada** para provocá-la: a prova da sua defesa continua sendo
`tests/test_context.py`, e a prova de que R1 **a captura** é **estrutural**, por AST sobre a
tupla fechada de `OMR1-11`.

**Reconciliação — item 3a (destino do alerta).** O **destino físico** do alerta operacional
continua **ABERTO** (§12, item 3a). R1 o trata como **dependência injetada**, não como
decisão sua: ele exige a **tentativa** e fecha o **payload sanitizado**, sem escolher canal.
Logo, o item 3a é **bloqueador da composição de produção / canal real** — que precisa de um
destino concreto — e **não** bloqueador do **runtime local R1**, que já existe e é
exercitado com a dependência fornecida pelo chamador.

**Reconciliação — item 18 (limiar temporal).** O **valor real** do limiar de recência e o
**mecanismo de carga** continuam **ABERTOS** (§12, item 18). R1 **transporta** o limiar
recebido e **não o revalida** (`OMR1-4`). Logo, o item 18 é **necessário antes da composição
de produção / canal real**, e **não** é **pré-requisito para a existência do R1**.

**Reconciliação — grafo de bloqueadores e `N-b`.** A **materialização em runtime** da
coordenação da etapa 4 (`CN4-1`–`CN4-12`, §6.3) é **escopo de recortes posteriores do
`OrquestradorMotor`** — e **não** pré-requisito circular do R1, que **não alcança a etapa
4**. O grafo do final de §12 continua descrevendo o que bloqueia o `OrquestradorMotor`
**completo** e a integração *end-to-end*, não o prefixo aqui materializado.

**Marcos.** Para impedir que "existe runtime" seja lido como "está pronto", três marcos são
registrados — e **nenhum deles é etapa, subetapa ou componente do pipeline**:

| Marco | Significado | Situação |
|---|---|---|
| **M1** | **runtime local parcial** do `OrquestradorMotor` — etapas 1–3 mais TS45 | **liberado por R1** |
| **M2** | **primeiro teste conversacional local** — ciclo ligado de ponta a ponta | **ainda NÃO liberado** |
| **M3** | **produção real** em canal, com persistência não volátil, destino de alerta e limiar carregado | **ainda NÃO liberada** |
...
| FE-12 | **S2-D8 continua responsável por cobertura e causas.** A **montagem** da fotografia deixou de ser *inline*: ela tem **uma única fronteira compartilhada**, `montar_fotografia_fragmento` (§4.4.6, **`FF-1`**–**`FF-12`**), que S2-D8 **consome**. O que continua sendo de S2-D8 é **decidir candidatura**, **resolver** o valor de cada `ASSERTIVA` de runtime a partir da fotografia factual autoritativa, **decidir cobertura** e **projetar** cada impedimento em `CausaE09` **com o assunto** — **com caminho** → `CAMPO_INDISPONIVEL`; **sem caminho** → `SEM_RESPOSTA_APROVADA_EMITIVEL`, ambos `ACESSORIA` (**`D8-E6`**) —, e que a **deduplicação** de **`D8-E4`** acontece. O comportamento público de `decidir_pendencias_e_cobertura` é **idêntico** ao anterior à extração. |
...
| FE-14 | **Escopo isolado — a primitiva NÃO é *owner* de `T16`/`R06`.** Por si só ela **não conhece `T16`** e **não conhece ação alguma**, e **não é *owner*** de `R06/F1`: a **entrega que a materializou não implementou T16**. A **rota de ação** materializada **depois**, em **§4.1.5** (**PE-15**–**PE-20**), passou a **consumir esta mesma autoridade** — e é para isso que ela é única —, mas o ***owner* continua sendo o `ProjetorEmissao`**, **nunca a primitiva**. Nada disso a torna *action-aware*: ela permanece **sem conhecer ação**, **sem produzir `E09`** e **sem produzir *handoff*** (**FE-7**), e **`D8-F` continua com implementação única** (**FE-2**). |
...
### 4.4.6 Contrato vivo da montagem compartilhada da `FotografiaFragmento`

**Esta seção não altera §4.4.1 e não renumera `D8-F`.** Ela separa, de vez, **montar** de
**avaliar**: a **decisão** de emissibilidade continua com **uma única implementação**,
`avaliar_emissibilidade` (§4.4.5); o que passa a existir aqui é **uma única fronteira de
montagem** da fotografia que aquela primitiva consome, **já materializada** em
`src/casa77_sdr/fragment_snapshot.py`, com os cenários em
`tests/test_fragment_snapshot.py`. A API pública é `montar_fotografia_fragmento`, sobre a
`FotografiaFragmento` **já existente** de §4.4.5. **§4.1 permanece com 14 componentes**, §2
com **nove** responsabilidades e §5 com **catorze** etapas; **nenhuma etapa nova**, **nenhuma
condição de §4.4 criada ou alterada**, **nenhum DTO, enum ou exceção pública nova** e
**nenhum evento novo**.

| # | Contrato |
|---|---|
| FF-1 | **Uma pergunta, e só ela**: *qual é a **fotografia** deste fragmento?* A fronteira é **pura**, **determinística** e **estrutural**. **Zero I/O**, *filesystem*, rede, LLM, SDK, YAML, `knowledge/**`, relógio, calendário, ambiente, *logging*, *cache*, *retry*, fila, *sleep* e mutação de entrada. Ela importa **apenas** `fragment_emissibility` e `response_consistency`. |
| FF-2 | **Entradas, e só elas.** `token` — o identificador estrutural de um fragmento **que já é candidato** —, `consistencia` — o `ResultadoConsistencia` **já produzido** pelo `ValidadorConsistenciaBase` — e `assertivas_runtime` — os pares `(predicado, valor)` **já resolvidos pelo chamador**. Esta terceira entrada é ***keyword-only* e obrigatória, sem valor padrão**: um `()` implícito faria um token com *binding* **`RUNTIME_AUTORITATIVO`** escapar **em silêncio** sem a sua fotografia factual. A fronteira **não recebe** índice, `fatos_runtime` nem a projeção de runtime por token. |
| FF-3 | **Saída única.** Uma `FotografiaFragmento` — a **mesma** de §4.4.5, reutilizada. **Nenhum DTO agregador, enum ou exceção pública nova** é criado, e **nada mais** é devolvido. |
| FF-4 | **Autoridade do status: `status_por_fragmento`.** O rótulo de **`C-3`** vem do par cujo token é o recebido, e **precisa existir exatamente uma vez**. **Zero *fallback***, **zero valor padrão**, **zero consulta ao índice** e **zero autoridade de status paralela**. A **seleção é pelo token**, nunca pela posição física. |
| FF-5 | **Autoridade da divergência: `tokens_divergentes`.** `divergente` é, literalmente, `token in consistencia.tokens_divergentes`. A fronteira **não reavalia `ASSERTIVA`**, **não relê `divergencias`** e **não reabre Classe II** (**`D8-CII`**, **`R2F-12`**). |
| FF-6 | **`C-7`: ordem física e zero deduplicação.** `referentes_indisponiveis` é a projeção de `consistencia.referentes_indisponiveis` filtrada por `item.token == token`, transportando **somente** `item.referente`, na **ordem física recebida**. Filtrar **não reordena**, e **duplicata é preservada**: reduzir perderia causa estrutural (**`D8-E4`**). **`mecanismo`**, **`binding`**, **`origem`**, **`rxx`** e **`fragmento_id`** **não entram na fotografia**. |
| FF-7 | **Runtime é transportado, nunca resolvido e nunca avaliado.** `assertivas_runtime` atravessa **literalmente**, preservando **ordem**, **duplicatas**, **predicado** e **`bool` exato** — `1`/`0` **fecham**, porque passariam por verdade booleana sem serem o fato resolvido. **Resolver** o valor continua sendo de **S2-D8**; **avaliá-lo** continua sendo de `avaliar_emissibilidade` (**`D8P-10`**). |
| FF-8 | ***Fail-closed* quando a fotografia não é projetável univocamente.** Token sem par de status → `status_ausente`; token com **mais de um** par → `status_ambiguo`. Forma inválida das entradas da própria superfície → `tipo_invalido`. **Sem resultado parcial** e **sem exceção pública nova**: são `TypeError` e `ValueError`, com mensagem **estrutural** `<categoria>: <localizador>` que **nunca** carrega o token recebido, o valor recebido, o `repr`, a cardinalidade, a posição, PII ou dado comercial. **Ela NÃO é, porém, um segundo `ValidadorConsistenciaBase`.** O `ResultadoConsistencia` é **entrada já produzida a montante**. Aqui **não** se revalida o domínio completo, o índice, o status canônico completo, **`VCB-7`**, **`VCB-10`**, *bindings* ou corpus, e **não** se julga a gramática `<Rxx>/<Fxx>` do token — a identidade canônica do domínio pertence às autoridades já existentes. Confere-se **apenas** o que a própria superfície exige e o mínimo estrutural necessário para ler o campo projetado. |
| FF-9 | **Zero `D8-F`: montar não é avaliar.** A fronteira **não** decide emissibilidade, **não** aplica `D8-F1`–`D8-F6`, **não** ordena causas e **não** produz `ImpedimentoEmissao`. **`D8-F` continua com uma única implementação de decisão**, `avaliar_emissibilidade` (**`FE-2`**), e ela é chamada **depois**, sobre a fotografia montada aqui. |
| FF-10 | **Dois consumidores autorizados, e nenhum outro por analogia.** **(1)** **S2-D8** (§4.4.3), que fotografa **cada alternativa candidata** que avalia, **antes da etapa 7**. **(2)** o **futuro `OrquestradorMotor`**, na rota *action-owner* de **T16**/`R06` (§4.1.5, **`PE-15`**–**`PE-20`**), **entre** a primeira decisão da máquina e a projeção de emissão. Esta fronteira **não tem uma única posição temporal no pipeline**: ela é **autoridade compartilhada**. **Nenhum terceiro consumidor herda autorização**, e cada novo exigiria arbitragem própria. |
| FF-11 | **Zero decisão comercial, evento, *handoff* ou máquina.** `AssuntoComercial`, `PerguntaComercial`, `CausaE09`, `MotivoE09`, `ClassificacaoPendencia`, `R2`, grupo, ***witness***, cobertura, candidatura, pendência, `Evento`, `AcaoMaquina`, `MotivoHandoff`, ação e `MaquinaEstados` **não entram aqui**. Ela **não** decide candidatura (**`D8-G`**, **`D8P-11`**), **não** decide cobertura, **não** resolve *binding*, **não** produz `E09` ou `E18` e **não** chama a máquina. |
| FF-12 | **Limites.** Materializar esta fronteira **não** integra o ciclo, **não** implementa o `OrquestradorMotor`, **não** conclui **T16** *end-to-end*, **não** cria fragmento, texto ou regra comercial e **não** altera `knowledge/**`. **A segunda invocação de FF-10 ainda não existe**: a chamada desta primitiva pela rota *action-owner* de T16 **dentro de um ciclo real** continua sendo responsabilidade **futura** do `OrquestradorMotor` (§6.3, **`CN4-1`**–**`CN4-12`**). |
...
- **a coordenação da etapa 4 no ciclo está normatizada** por **`CN4-1`**–**`CN4-12`** (§6.3):
  **um único** ponto de chamada de `executar_interpretacao_do_ciclo(...)` por ciclo de nova
  mensagem, o **`ArtefatosInterpretacao` reutilizado** pelos consumidores posteriores, a
  **ordem parcial** obrigatória entre os produtores e as **pré-condições** da primeira
  decisão. É **norma do `OrquestradorMotor` futuro**, **não** implementação: ela **não cria
  etapa** — o pipeline continua com **catorze** — e **não** acrescenta linha a esta tabela;
...
#### Fronteira operacional da cadeia N-b

Contrato vivo da **junta física** entre a **etapa 4** e os **três consumidores
determinísticos** que já existiam: `src/casa77_sdr/interpretation_stage.py`, com os
cenários em `tests/test_interpretation_stage.py`. Ela **não reabre** N-b, AJ1–AJ4,
`N-b-RES2` nem a máquina, **não cria componente** — §4.1 permanece com **14**, e o
pipeline permanece com **catorze** etapas —, **não cria evento**, **não cria condição**,
**não cria etapa** e **não cria subetapa**. Ela **não é** o `OrquestradorMotor`.

| # | Contrato de implementação EN4 |
|---|---|
| EN4-1 | **Owner e posição.** `src/casa77_sdr/interpretation_stage.py` compõe, numa única invocação, a produção da `Interpretacao` (etapa 4) e as **três** derivações determinísticas que a consomem. Ele é **fronteira operacional**, não etapa nova: a **etapa 4 continua sendo a etapa 4**, e a co-locação é **física**, não semântica. Superfície pública de **exatamente dois** nomes — `ArtefatosInterpretacao` e `executar_interpretacao_do_ciclo(...)` —, **não exportada** pelo `__init__.py` do pacote. **Nenhum enum novo** e **nenhuma exceção pública nova**. |
| EN4-2 | **API.** `executar_interpretacao_do_ciclo(mensagem: str, *, produtor: ProdutorTextoEstruturado, prompt_sistema: str) -> ArtefatosInterpretacao`. A dependência é o **protocolo** `ProdutorTextoEstruturado`, **nunca** o `AdaptadorAnthropic`: a fronteira **não cria cliente**, **não lê ambiente**, **não lê `.env`**, **não escolhe** fornecedor, modelo, SDK ou credencial e **não lê o prompt do filesystem**. `prompt_sistema` chega como **argumento explícito**, e o **mecanismo de carga** do prompt permanece **fora do escopo**. |
| EN4-3 | **Uma chamada por invocação.** Cada invocação bem formada executa `interpretar_mensagem(...)` **exatamente uma vez** e, por consequência, provoca **no máximo uma** chamada ao `ProdutorTextoEstruturado`. **Zero laço**, zero retry, zero *fallback*, zero cache e zero fila em torno dela — a prova é **sintática**, por AST, além de comportamental. O risco de duplicação **dentro da cadeia** fica fechado. |
| EN4-4 | **Unicidade por ciclo é obrigação futura.** Esta fronteira **não prova**, sozinha, "uma chamada de LLM por ciclo completo": o `OrquestradorMotor` **não existe** e, teoricamente, poderia invocá-la duas vezes. Fica registrada a obrigação: **o `OrquestradorMotor` deverá invocar esta fronteira no máximo uma vez por ciclo de nova mensagem.** Até lá, a unicidade **global** por ciclo depende da **coordenação futura**. |
| EN4-5 | **A mesma `Interpretacao` atravessa as três derivações.** A instância devolvida pela única chamada da etapa 4 é a que chega a `projetar_para_identidade(...)`, a `decidir_interesse_confirmar_disponibilidade(...)` e a `produzir_eventos_da_interpretacao(...)`, e é a que fica no DTO — provada por **identidade de objeto**. **Nenhuma segunda interpretação da mesma mensagem é produzida**, e **nenhuma regra é copiada, revalidada ou recanonicalizada** aqui: as validações permanecem dentro de cada fronteira chamada. |
| EN4-6 | **Ordem canônica.** `interpretar_mensagem` → `projetar_para_identidade` → `decidir_interesse_confirmar_disponibilidade` → `produzir_eventos_da_interpretacao` → DTO. A produção da `Interpretacao` **antes** das derivações é **obrigatória**. A ordem relativa entre as **três derivações** é **operacional e auditável** e **não** expressa precedência semântica. |
| EN4-7 | **DTO.** `ArtefatosInterpretacao`, `frozen` e com `slots`, com **exatamente quatro** campos: `interpretacao`, `projecao_identidade`, `eventos_interpretacao` e `interesse_confirmar_disponibilidade`. A `Interpretacao` fica no DTO **justamente** para que os consumidores posteriores reutilizem **o mesmo objeto**, sem reinterpretar a mensagem. |
| EN4-8 | **PII e `repr`.** `interpretacao` e `projecao_identidade` são declarados `field(repr=False)`: o primeiro transporta texto conversacional e PII de runtime; o segundo, o **tipo** e a **data nominais** do evento. O `repr` do DTO expõe **somente** os eventos confirmados e o booleano da condição 5 — **nenhum** nome, contato, pergunta comercial, trecho ambíguo, tipo nominal ou data nominal. **Nenhum `__repr__` manual** é escrito. |
| EN4-9 | **Sem `Interpretacao`, nada é derivado.** Se `interpretar_mensagem(...)` levantar `FalhaProdutorInterpretacao`, a exceção **propaga intacta**: a fronteira **não captura**, **não converte**, **não retorna DTO**, **não chama** derivação alguma, **não chama** identidade, **não chama** a etapa 6, **não chama** a composição de §4.1.9 e **não chama** a `MaquinaEstados`. **Nenhum campo `None` é fabricado** num DTO parcial. **`N-b-M1`**/**`N-b-M2`** continuam valendo: sem `Interpretacao`, a **etapa 5 não executa** e o ciclo **não alcança** as etapas 6 e 7 — em particular, `montar_condicoes_ciclo(...)` **não é chamada** com uma condição 5 ausente. `decidir_interesse_confirmar_disponibilidade(None) is None` permanece contrato da função existente e **não é exercido** por este caminho. |
| EN4-10 | **Erros de contrato propagam intactos.** `TypeError` e os `E-Nb-*` como `ValueError` **não são capturados**, **não viram `FalhaProdutorInterpretacao`**, **não viram ausência silenciosa**, **não viram `Identidade.AMBIGUA`** e **não produzem DTO parcial**. **Zero taxonomia nova** é criada. |
| EN4-11 | **Co-locação física sem redefinir a etapa 4.** `produzir_eventos_da_interpretacao(...)` continua sendo **`N-b-RES2`**: **posterior** à etapa 4 e **fora** dela. **`N-b-G2`**, **`N-b-RES1`**, **`N-b-RES3`** e **`RES2-1`**–**`RES2-12`** permanecem **literais** — a etapa 4 **continua não emitindo `Exx`**. O que esta fronteira faz é **encadear fisicamente** produção e derivação num único artefato operacional. **Produzir não é consumir**: o coordenador futuro pode **descartar** esses eventos se o ciclo terminar antes da etapa 7. |
| EN4-12 | **Pureza e fora do escopo.** O **único** efeito externo permitido é a chamada ao `ProdutorTextoEstruturado` realizada **dentro** de `interpretar_mensagem`. A fronteira **não** acessa persistência, YAML, `knowledge/**`, filesystem, rede direta, relógio, aleatoriedade, logging, cache, retry, fila, *sleep*, SDK ou variável de ambiente, e **não** emite mensagem, alerta ou resposta. **Não chama**: `detectar_handoff`, `decidir_encerramento`, `atualizar_dados_atendimento`, `decidir_pendencias_e_cobertura`, `produzir_eventos_internos_ciclo`, `agregar_eventos_primeira_decisao`, `montar_condicoes_ciclo`, a resolução de identidade e `decidir`. **Sequenciar o pipeline continua sendo papel do `OrquestradorMotor` futuro**, que permanece **ausente**. |

#### Coordenação de N-b no ciclo

Contrato vivo da **coordenação por ciclo** da cadeia N-b: **quando** a fronteira operacional
de `EN4-1`–`EN4-12` é invocada num ciclo de **nova mensagem**, **quantas vezes**, o que o
ciclo faz com o artefato que ela devolve e o que acontece quando ela falha. É **norma do
`OrquestradorMotor` futuro**, e **não** implementação: nada aqui é materializado, e
`src/casa77_sdr/interpretation_stage.py` permanece **inalterado**. CN4 **não reabre** N-b,
AJ1–AJ4, `N-b-RES2`, `EN4`, `S2-D8`, `CAL6`, `E4` nem a máquina; **não cria** componente —
§4.1 permanece com **14** —, **não cria** etapa nem subetapa — o pipeline permanece com
**catorze** (§5) —, **não cria** responsabilidade — §2 permanece com **nove** —, e **não
cria** DTO, enum, exceção, evento ou condição.

| # | Contrato de coordenação CN4 |
|---|---|
| CN4-1 | **Owner.** A coordenação por ciclo é do **`OrquestradorMotor`**, e de mais ninguém. É consequência direta de **D1** — ele conhece todos os componentes e nenhum componente o conhece —, de **D7** — coordenação entre fronteiras independentes **sobe** para o orquestrador — e de **EN4-4**, que já registrava a obrigação como futura. CN4 **transforma essa obrigação em norma escrita**, sem materializá-la. **Nenhum componente novo, etapa, subetapa, DTO, enum ou exceção é criado**: §4.1 continua com **14** componentes, §2 com **nove** responsabilidades e §5 com **catorze** etapas. |
| CN4-2 | **Ponto único da interpretação.** Num ciclo de **nova mensagem** que **legitimamente** alcançou a etapa 4, a sequência normativa é `etapa 3 concluída` → `executar_interpretacao_do_ciclo(...)` → `etapa 5`. A chamada ocorre **depois** da recuperação **válida** de contexto (§6.2) e **antes** da resolução de identidade (§7.1), preservando a ordem essencial de §5. O `OrquestradorMotor` futuro deve possuir **exatamente um** ponto **sintático e estrutural** de chamada a `executar_interpretacao_do_ciclo(...)` por ciclo de nova mensagem. Quando o ciclo **termina antes da etapa 4** — duplicata na etapa 2, contexto inválido na etapa 3 —, o número de chamadas é **zero**. **Os demais tipos de ciclo não são especificados aqui** (CN4-12). |
| CN4-3 | **Mecanismo de unicidade, e só ele.** A garantia futura é **estrutural**: **um** ponto de chamada, **uma** variável local guardando o `ArtefatosInterpretacao` devolvido, e **reutilização desse mesmo resultado** por todos os consumidores. São **explicitamente proibidos** como mecanismo: *flag*, *cache*, estado persistido, *retry*, fila, *lock*, DTO novo e **segunda interpretação** da mesma mensagem. **`EN4-3` continua garantindo uma interpretação por invocação da fronteira**; **CN4 fecha o degrau que faltava**: **no máximo uma invocação da fronteira por ciclo de nova mensagem**. Nada disso altera `interpretation_stage.py`. |
| CN4-4 | **Artefato transportado como unidade.** O resultado continua sendo o `ArtefatosInterpretacao` de **EN4-7**, e o coordenador futuro o **transporta inteiro**. Ele **não** é desmontado para construir um DTO agregador novo e **nenhuma estrutura intermediária** é criada. Os campos consumidos são **exatamente os quatro** já existentes: `artefatos.projecao_identidade`, `artefatos.interpretacao`, `artefatos.eventos_interpretacao` e `artefatos.interesse_confirmar_disponibilidade`. |
| CN4-5 | **Mapa de consumidores, com identidade de objeto preservada.** A **etapa 5** — resolução de identidade — consome `artefatos.projecao_identidade`. Consomem **a mesma instância** de `artefatos.interpretacao`: o **`AtualizadorDadosAtendimento`** (`atualizar_dados_atendimento`, §4.1.8), **S2-D8** (`decidir_pendencias_e_cobertura`, §4.4.1), o **`DetectorHandoff`** (`detectar_handoff`, `DH-1`–`DH-12`) e **S3-D1** (`decidir_encerramento`, `S3D1-1`–`S3D1-12`). A **agregação** de §4.1.9 recebe `artefatos.eventos_interpretacao` como o parâmetro `eventos_interpretacao` de `agregar_eventos_primeira_decisao(...)`, e a **condição 5** de §4.4 recebe `artefatos.interesse_confirmar_disponibilidade` em `montar_condicoes_ciclo(...)`. A **identidade de objeto** da `Interpretacao` é preservada em todo o percurso, exatamente como **EN4-5** já exige dentro da fronteira. **Nenhuma dessas fronteiras reinterpreta a mensagem**, e nenhuma delas chama o produtor. |
| CN4-6 | **Ordem parcial canônica — e ela é parcial de propósito.** CN4 fixa **apenas** as dependências **obrigatórias**, e **não** converte produtores independentes numa sequência semântica total. **(A)** etapas **1**–**3** → fronteira da **etapa 4** (`interpretation_stage`) → **etapa 5** (identidade). **(B)** depois da identidade, a continuação normal preserva **literalmente** a ordem conceitual determinística já vigente de §5 e §4.4.1: `AtualizadorDadosAtendimento` → `RegrasComerciais` → **qualificação provisória** → **S2-D8** → **qualificação final**. **(C)** produtores: o **`DetectorHandoff`** pode executar em **qualquer ponto** entre a existência de uma `Interpretacao` válida e a agregação de eventos — a sua posição relativa a `data_update`, `RegrasComerciais` e **S2-D8** **não é fixada** e **não é precedência semântica**; **S3-D1** exige `Interpretacao` **e** o resultado da **qualificação final**, logo ocorre **depois** dela; `produzir_eventos_internos_ciclo(...)` exige **qualificação final**, `insumo_qualificacao_atualizado` e `ResultadoS2D8`, logo ocorre **depois** dessas três autoridades. **(D)** junta final: **somente** quando todos os produtores necessários já entregaram, ocorrem `agregar_eventos_primeira_decisao(...)` e `montar_condicoes_ciclo(...)`, e **só então** a **primeira** chamada da `MaquinaEstados` — a **etapa 7**. A **ordem da tupla de eventos continua sendo auditabilidade, nunca precedência** (`IC-1`–`IC-12`). |
| CN4-7 | **Pré-condições da primeira decisão.** A primeira chamada `decidir(estado, eventos, qualificacao, condicoes)` (§4.5) só ocorre depois de existirem, **todos**: o **estado vigente** recuperado (§6.2, **D9**); a **identidade resolvida** ou a estrutura válida correspondente da etapa 5, inclusive o caso `HUMANO_UNICO` com `identidade = None`; a **qualificação FINAL**, nunca a provisória; os **eventos confirmados já agregados**; a **`CondicoesCiclo` validamente montada**; e **todas** as entradas que as autoridades próprias de cada insumo exigirem. A **assinatura da máquina não é alterada**. `None` legítimo **não** é convertido em `False`: `None`, `False` e `()` permanecem **estados distintos** ao longo de toda a coordenação (`IC-1`–`IC-12`). |
| CN4-8 | **Falha da interpretação bloqueia o caminho normal.** Se `executar_interpretacao_do_ciclo(...)` falhar — `FalhaProdutorInterpretacao`, `TypeError`, ou `ValueError` dos `E-Nb-1`–`E-Nb-19` —, a exceção **propaga intacta** pelo caminho normal, como **EN4-9** e **EN4-10** já determinam. O coordenador **não** cria resultado parcial, **não** monta `ArtefatosInterpretacao` parcial, **não** fabrica `Identidade.AMBIGUA` sintética, **não** fabrica condições, **não** cria *fallback* interno e **não** cria taxonomia nova. No caminho normal, **não executam**: a **etapa 5**, a **etapa 6**, **S2-D8**, o **`DetectorHandoff`**, **S3-D1**, `produzir_eventos_internos_ciclo(...)`, `agregar_eventos_primeira_decisao(...)`, `montar_condicoes_ciclo(...)` e `decidir(...)`. **`N-b-M5` é preservado literalmente: nada derivado de interpretação inexistente é gravado** — e **nada além disso é afirmado aqui** sobre gravação ou emissão, que continuam regidas por §7 e §7.2. |
| CN4-9 | **Modo degradado preservado, não implementado.** **`N-b-M3`**, **`N-b-M4`**, **`N-b-M5`**, **`N-b-M6`**, **`N-b-M7`** e **`N-b-M8`** permanecem **integralmente**. Em especial, **`N-b-M4`** continua valendo sob a política já existente de §7: **sem extração nova**, **textos aprovados literais**, mensagem que exija interpretação → **R03 + handoff**, e **nunca adivinhar**. **CN4 não decide e não implementa esse desfecho.** A **coordenação operacional do modo degradado** permanece **futura** e pertence ao `OrquestradorMotor` (**`N-b-M8`**). Qualquer emissão continua sujeita a **§7.2**. |
| CN4-10 | **Alerta.** **CN4 não cria alerta**, **não cria gatilho de alerta** e **não escolhe destino**. **`N-b-M3` permanece literal**: preservação e alerta seguem **somente** os contratos já vigentes de §7 e §7.2 e a coordenação futura. **Nenhum** canal, serviço, banco ou fila é escolhido. O **item 3a de §12 continua ABERTO**. Isto **não** afirma que não haverá alerta: afirma apenas que **esta coordenação documental não define nem materializa o tratamento operacional do alerta**. |
| CN4-11 | **E4 está fora do escopo e permanece intacta.** **`E4-1`**–**`E4-14`** (§7.1) são **preservadas integralmente**, e **CN4 não coordena E4**. O fluxo é: a **etapa 4** produz `ArtefatosInterpretacao`; a **etapa 5** consome `artefatos.projecao_identidade`; e é o **`ResolvedorIdentidade`** que conclui `SEM_CANDIDATO_ELEGIVEL`. **Não é verdade** que o artefato seja descartado sem consumo — a projeção **já foi consumida** na etapa 5. A formulação correta é: quando **E4** encerra o ciclo, **não há consumo POSTERIOR à etapa 5** dos artefatos destinados às etapas seguintes. Logo: a **etapa 6 não executa**; a `Interpretacao` **não segue** para `data_update`, **S2-D8**, `DetectorHandoff` ou **S3-D1**; os **eventos de interpretação não seguem** à agregação; a **condição 5 não segue** a `CondicoesCiclo`; e a **máquina não é chamada**. O tratamento operacional — processamento pendente preservado, **tentativa** de alerta, chave de idempotência marcada e **encerramento sem transição** — permanece **integralmente** sob `E4-1`–`E4-14`. |
| CN4-12 | **Limites.** Ficam **fora** desta arbitragem: a **implementação** do `OrquestradorMotor`; a **etapa 3** e **N-a**; o **valor** do limiar temporal e o **mecanismo de carga** dele; o tratamento operacional de **S4**/**S5**; o **destino do alerta**; a **persistência não volátil**; o **adaptador real de calendário**, o Google Calendar e o WhatsApp; o **produtor real** da `FotografiaFragmento` e de `fatos_runtime`; a **integração *end-to-end***; as **chamadas da máquina não originadas de nova mensagem**; e **`E15`**, **`E12`** e **`E13`**. **`calendario_integrado` não é produzido por CN4**: o coordenador futuro apenas o **transporta**, conforme **`CAL6-1`**–**`CAL6-8`**. |
...
#### Contrato TS45 — tratamento operacional dos bloqueios S4/S5 da etapa 3

Arbitragem do **primeiro desfecho** da lista acima, que o doc 06 §4.5 enumera como o **caso
1** dos quatro encerramentos sem transição. Ele responde a **uma** pergunta e nada mais —

> quando a **etapa 3** bloqueia sob **S4**/**S5**, quais efeitos o futuro
> `OrquestradorMotor` executa, e quais são **proibidos**?

O contrato é **normativo e conceitual**: ele **não cria componente** — §4.1 permanece com
**14** —, **não cria etapa nem subetapa** — o pipeline permanece com **catorze** (§5) —,
**não cria** estado, evento, condição, transição, DTO, enum ou exceção, **não cria arquivo**
em `src/`, **não altera** `persistence.py` e **não autoriza implementação**. **`S1`**–**`S11`
permanecem literais**, e **`E4-1`**–**`E4-14`** não são reabertos. A **coordenação** destes
efeitos no pipeline pertence ao `OrquestradorMotor`, que permanece **ausente**.

**Gatilho fechado — exatamente sete exceções nomeadas.** O futuro runtime deve capturar as
**classes nomeadas**, e **nunca** um `except ValueError:` genérico como captura de TS45:

| Exceção | Atribuição normativa |
|---|---|
| `ConfiguracaoTemporalInvalida` | **S10** — erro de contrato da configuração |
| `IdentificadorNaoResolvido` | **N5**, **N6**, **S3** |
| `ContextoElegibilidadeCorrompido` | **S11** |
| `MarcoTemporalAusente` | **S9** |
| `IdentificadoIncoerente` | **N-a-F1**, **N-I-2**, **P-I5** |
| `ConjuntoHumanoIncoerente` | **H4**, **H5** |
| `ProjecaoIdentificadorIncoerente` | **N-I-4** |

| # | Contrato TS45 |
|---|---|
| TS45-1 | **Natureza.** Norma **documental** do futuro `OrquestradorMotor`. **Nenhum componente, etapa, subetapa, estado, evento, condição, transição, DTO, enum ou exceção nova** é criado; `src/**`, `tests/**`, `knowledge/**` e `prompts/**` não são tocados; e o **`OrquestradorMotor` continua ausente**. |
| TS45-2 | **Gatilho fechado.** TS45 aplica-se **exclusivamente** às **sete** exceções nomeadas da tabela acima, com a atribuição normativa de cada uma **preservada**. O conjunto é **fechado**: nenhuma outra falha entra por analogia, e a captura futura é **por classe nomeada** — **`except ValueError:` genérico é proibido** como captura de TS45. |
| TS45-3 | **Mesmo desfecho, causa preservada.** Os sete bloqueios produzem o **mesmo desfecho observável** — preservar, tentar alertar, marcar a chave, encerrar sem transição. A **distinção de causa** é preservada **para auditoria**, e **não** vira enum, DTO, motivo de *handoff* ou evento. |
| TS45-4 | **Processamento pendente — DTO existente.** O pendente é `ProcessamentoPendente(canal, contato, conteudo)` de `src/casa77_sdr/persistence.py`, com **exatamente os três campos existentes**. **Zero campo novo**: não entram causa, *timestamp*, contador, status, `retry_id`, *stack trace*, `repr` de exceção, `id_atendimento` ou *payload* paralelo. |
| TS45-5 | **Conteúdo preservado.** `conteudo` é a **`EntradaNormalizada.mensagem_normalizada`** produzida pela **etapa 1**, e **não** a `EntradaMensagem.mensagem` bruta. A etapa 1 já executou antes da etapa 3; a forma normalizada é a **representação técnica canônica** do motor; a normalização é **pura, determinística e conservadora**; e preservá-la evita **duas representações concorrentes** do mesmo conteúdo operacional. **Isto não é afirmação de redução semântica de PII** e **não define em que etapa um replay futuro começaria**. |
| TS45-6 | **Preservação obrigatória.** Nos **sete** bloqueios, preservar o processamento pendente é **obrigatório** (**S4**). A mensagem não é descartada, e o ciclo não termina sem que essa tentativa tenha ocorrido. |
| TS45-7 | **Ordem normativa.** **1.** detectar o bloqueio TS45; **2.** preservar o `ProcessamentoPendente`; **3.** **tentar** o alerta operacional; **4.** marcar a chave de idempotência; **5.** encerrar o ciclo **sem transição**. |
| TS45-8 | **Marcação da chave.** A chave é marcada **fora da etapa 13** e **somente após preservação bem-sucedida**, por `marcar_chave_processada`, que **já existe**. §4.3 **não** condiciona a marcação à existência de transição; sem ela, cada reentrega técnica repetiria preservação e alerta, e o `ProcessamentoPendente` **não possui mecanismo de deduplicação**. **Antes** da conclusão do tratamento, a semântica permanece ***at-least-once***. |
| TS45-9 | **Reentrega técnica × reprocessamento deliberado.** São operações **distintas**. A **reentrega técnica externa** origina-se do canal, de retentativa ou de reinício, passa **normalmente** pela **etapa 2** e, com a chave já marcada, **encerra sem novo efeito**. O **reprocessamento deliberado** de um `ProcessamentoPendente` preservado é matéria **futura**, e o mecanismo concreto deverá conseguir **consumir um pendente preservado sem depender de uma nova entrega externa do canal**. **`E4-13` permanece integralmente preservado.** |
| TS45-10 | **Replay concreto fora do escopo.** **Nenhum** *scheduler*, fila, *cron*, *worker*, *retry* automático, consumidor, expurgo ou limite de tentativas é escolhido, e **nenhum executor desse tipo é afirmado como existente**. TS45 **não** define em que ponto do pipeline um replay deliberado entraria, **não** cria chave nova, **não** desmarca nem reutiliza chave, e **não** afirma que ele contornaria a etapa 1 ou a etapa 2. |
| TS45-11 | **Falha ao preservar.** Se `preservar_pendente(...)` falhar: a chave **não é marcada**; o **alerta operacional é tentado** mesmo assim; **zero emissão**, **zero atendimento**, **zero handoff**, **zero máquina**; e o tratamento **não é considerado concluído**. Uma **reentrega externa futura pode repetir o tratamento**, sob ***at-least-once*** — a prioridade é **não perder a mensagem**. **Nenhuma exceção pública nova** é criada e **nenhuma tecnologia de persistência é escolhida**. |
| TS45-12 | **Falha do alerta após preservação.** Com o pendente **já preservado** e a **tentativa de alerta falhando**: a **chave é marcada**; **zero resposta**, **zero atendimento**, **zero handoff**; **zero retry de alerta**, **zero fila**, **zero contador** e **zero status de entrega**; e o ciclo **encerra sem transição**. **A reentrega externa não é retry do alerta.** |
| TS45-13 | **Falha ao marcar a chave.** Com o pendente **já preservado** e o alerta **já tentado**, se `marcar_chave_processada` falhar: o **pendente permanece protegido**; **zero emissão**, **zero atendimento**, **zero handoff**; e o tratamento **não é considerado plenamente concluído**. Uma reentrega externa **pode repetir** pendente e alerta, sob ***at-least-once***. **Nenhum retry automático é criado**, **nenhuma classe de exceção é escolhida** e `persistence.py` **não é alterado**. |
| TS45-14 | **Alerta — conteúdo sanitizado.** Para os sete bloqueios, a **tentativa** de alerta é **obrigatória**, em **caminho separado da conversa** (**S5**, **Q5**). Ele transporta **somente** informação estrutural suficiente para auditoria: a **categoria estrutural** do bloqueio, o **sucesso ou falha da preservação** e uma **correlação opaca** — a **chave de idempotência** serve, porque é **opaca por construção**. **Proibidos** no alerta: texto integral da mensagem, valor comercial, nome próprio desnecessário, `id_atendimento`, `repr` de exceção, *stack trace*, segredo e credencial. |
| TS45-15 | **Transporte do alerta continua aberto.** TS45 **não escolhe** Slack, e-mail, WhatsApp, SMS, *webhook*, provedor, SDK, DTO externo, formato, SLA ou confirmação de entrega. O **item 3a de §12 continua ABERTO**. Registra-se apenas que a **mesma futura fronteira de tentativa de alerta poderá atender E4 e TS45**, desde que **preserve os dois contratos** — e **essa fronteira não é criada aqui**. |
| TS45-16 | **Owner futuro.** Os efeitos de TS45 são executados pelo **`OrquestradorMotor`**, e por mais ninguém. Nenhuma fronteira existente passa a executá-los, e **nenhuma delas é alterada** por esta arbitragem. |
| TS45-17 | **Negações fechadas.** Sob os sete bloqueios: a **`MaquinaEstados` não é chamada**; **nenhum evento** é produzido; **nenhuma transição** ocorre; **zero resposta automática**; **zero handoff**; **nenhum atendimento é criado**; a **etapa 5**, a **etapa 13**, a construção de **`CondicoesCiclo`** e o consumo de **S2-D8** **não ocorrem**; e **`instante_ultima_transicao` não é atualizado**. **Nenhum quinto caso de negócio é criado**: contexto inválido continua sendo o **caso 1** dos **quatro** encerramentos sem transição (doc 06 §4.5). |
| TS45-18 | **Etapa 4 sob bloqueio — decisão do MVP.** **`S8` permanece vigente**: a interpretação linguística **pode** rodar sobre contexto inválido **apenas para diagnóstico**. No **MVP**, entretanto, a **etapa 4 não executa** sob TS45 — logo **zero chamada de LLM**, **zero interpretação** e **zero custo de LLM** neste caminho. **Isto não revoga `S8`**: a **faculdade diagnóstica permanece disponível** para arbitragem e implementação futuras; o MVP simplesmente **não a exerce**. |
| TS45-19 | **Falha futura de leitura da persistência — fora do gatilho vigente.** A persistência não volátil futura poderá introduzir uma **falha explícita de leitura**, mas **nenhuma classe, taxonomia ou comportamento é inventado aqui**: hoje **não existe exceção pública concreta** para falha de `recuperar_por_id` ou `consultar_por_contato`. Quando essa fronteira existir, a sua **relação com TS45 deverá ser arbitrada especificamente**. Isso **não altera** o conjunto fechado de **sete** gatilhos, e **`E5`/`S7` não são usados para incorporá-la silenciosamente** ao contrato atual. |
| TS45-20 | **Limites.** Continuam **futuros e fora** desta arbitragem: o **destino do alerta operacional** (item 3a), o **limiar temporal** (item 18), a **persistência operacional não volátil** (item 2a), o **mecanismo concreto de replay** dos pendentes, a **implementação do `OrquestradorMotor`** e a **integração *end-to-end***. |
...
| 3a | Destino do alerta operacional não definido | S5, Q5 e F4 exigem um canal separado da conversa; o **destino operacional ainda não está especificado**. **ABERTA**. O recorte **R1** do `OrquestradorMotor` (§4.1.10) **não o escolhe**: ele recebe a **tentativa de alerta como dependência injetada** e fecha o **payload sanitizado** (`OMR1-5`, `OMR1-6`). Logo o item **bloqueia a composição de produção / canal real**, e **não** a existência do **runtime local R1** | etapa 5 / etapa 8 |
...
| 22 | **Montagem final da resposta e política da etapa 10** — o que **já está materializado isoladamente** e o que **ainda falta** | **PARCIALMENTE RECONCILIADA.** **Arbitrado e vigente** (§4.1.5): a **política geral de composição** **`PC-1`**–**`PC-6`**; a **ordem das unidades**, que preserva a produzida por `SF-D4`; o **fragmento de lacuna ao final** em cobertura mista (**PC-4**); o **separador `"\n\n"`** (**PC-3**); e **`PC-5`**, que tira do LLM a alteração lexical do **corpo comercial composto por fragmentos aprovados** (§4.2). **Arbitrado E materializado isoladamente**: o *renderer*/compositor **por fragmento** (§4.1.4), a **projeção dos fragmentos destinados à emissão** (§4.1.5) e a **montagem canônica de uma emissão** na mensagem única, **com a aplicação física do separador** (**§4.1.6**, `MF-1`–`MF-10`), e o ***gate* final de integridade textual** (**§4.1.7**, `VR-1`–`VR-10`) — **fronteiras isoladas**, sobre sequência **já decidida**. **Também materializados isoladamente**: a **infraestrutura estrutural de `R2`** (**§4.4.2**) e o **produtor determinístico de S2-D8** (**§4.4.3**). **Também materializados**: o **conteúdo aprovado de `knowledge/mapa-cobertura.yaml`** e a **aplicabilidade determinística de pacote** (**§4.4.4**). **Ainda NÃO materializados**: a **integração *end-to-end*** que liga a montagem e a validação ao ciclo, o **`OrquestradorMotor`**, a **invocação da `FotografiaFragmento` no ciclo** — a **primitiva determinística de montagem** está **materializada** (§4.4.6, `FF-1`–`FF-12`); o que falta é **quem a chama, quando** —, as **superfícies conversacionais residuais ainda sem fragmento aprovado** e o **papel residual correspondente do LLM** na etapa 10. **Sub-lacunas FECHADAS**: **T15** passou a ter **superfície aprovada determinística nas duas rotas** — `R05`/`F1` chega por **cobertura** quando S2-D8 já o autorizou, e pela **ação** quando não chegou (**PE-13**), com **`R05`/`F2`** e **`R05`/`F3`** fechando por contradição (**PE-14**); e **T16** passou a ter ***owner* de emissão isolado** — **`R06`/`F1`** em **dupla rota condicionada**, com **regra própria** e **zero analogia com R05**, a **cobertura** prevalecendo quando já trouxe o token e a **ação** consumindo **`avaliar_emissibilidade`** (§4.4.5) quando não trouxe, e **zero fragmento** sob **PE-7** quando **não emitível**, com **zero `E09`**, **zero `E18`** e **zero nova chamada da máquina** (**PE-15**–**PE-20**). **`T16` NÃO está integralmente concluída**: o ***owner* de emissão isolado não é a integração**, e continuam **abertos** a **invocação da `FotografiaFragmento` no ciclo** — a **primitiva de montagem** está **materializada** (§4.4.6), a **chamada dela pela rota *action-owner* de T16 não** —, a **coordenação pelo `OrquestradorMotor`**, a **integração *end-to-end*** e a **política da etapa 10 para a obrigação degradada**. **Ainda pendentes**: a **coleta**, o **formato** e a **retomada**, a **incompatibilidade dependente de motivo**, o **reforço de encaminhamento**, as **ações de chamadas posteriores** da máquina, o **papel residual do LLM** e as **demais superfícies conversacionais** de §4.2, para as quais a obrigação conversacional permanece (**PE-7**). **A integração *end-to-end* não está concluída** | materialização futura da **integração *end-to-end***, do **`OrquestradorMotor`**, da **invocação da `FotografiaFragmento` no ciclo** — a **primitiva de montagem** já **não é futura** (§4.4.6) —, das **superfícies conversacionais residuais** e do **papel residual correspondente da etapa 10**. A **infraestrutura estrutural de `R2`** (§4.4.2), o **conteúdo aprovado do mapa**, o **produtor determinístico de S2-D8** (§4.4.3) e a **aplicabilidade de pacote** (§4.4.4) **não são futuros**: já estão materializados. **`P4`** e o **`ValidadorResposta`** **já estão materializados isoladamente** (§4.1.7, item 7). **Não bloqueia** §4.1.4, §4.1.5, §4.1.6 nem §4.1.7, que são **fronteiras isoladas** e **não fecham o ciclo** |
...
**Grafo de bloqueadores ainda abertos.** O **`OrquestradorMotor` completo** continua
bloqueado pela **integração da etapa 4 no ciclo**, de **N-b** (item 12), pela **configuração
do limiar** de **N-a** (item 18) e pelo **destino do alerta** (item 3a). O **tratamento
operacional dos bloqueios `S4`/`S5`** **deixou de ser decisão arquitetural aberta**: ele está
**arbitrado** em §7.1 (**`TS45-1`**–**`TS45-20`**), e o seu **runtime deixou de estar
integralmente ausente** — ele está **materializado no recorte `R1`** (§4.1.10,
`OMR1-11`–`OMR1-13`).

**O grafo não é circular para o `R1`.** Os três itens acima condicionam o
`OrquestradorMotor` **completo** e a integração *end-to-end*, **não** o prefixo já
materializado: a **coordenação em runtime de N-b** é **escopo de recortes posteriores** do
mesmo componente, e R1 **não alcança a etapa 4**; o **limiar** (item 18) é **transportado**
por R1 e **necessário antes da produção**; e o **destino do alerta** (item 3a) é
**dependência injetada** em R1 e **necessário antes da produção**. **Arbitrar não é
materializar**, e **materializar um prefixo não é integrar o ciclo**.
