...
| DH-12 | **Pureza e fronteira.** Zero I/O, filesystem, rede, relógio, YAML, `knowledge/**`, LLM, SDK, persistência, logging, cache, retry e *sleep*; a entrada **não é mutada** e **nenhum texto da mensagem é lido**. **Zero palavra-chave, regex, score de sentimento ou contagem.** O detector **não produz** condição de §4.4, qualificação, ação, resposta, encerramento nem persistência, e **não faz a integração do ciclo**. Unir este `E18` aos eventos de **produtores distintos** é papel do **`OrquestradorMotor` futuro**, que **continua ausente**. |
