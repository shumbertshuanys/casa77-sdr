...
| PE-6 | **Duas unidades materializadas nesta versão, com naturezas distintas.** **`R03`/`F1`** é **mandatório puro**: ele só chega pela **ação de lacuna**, e nunca por cobertura. **`R05`/`F1`** é **unidade de dupla rota** para **T15** (**PE-13**): a **cobertura prevalece** quando já o trouxe, e a **ação o completa** quando não trouxe. Ambos são admissíveis porque um **gate mecânico do corpus versionado** prova que existem, que o rótulo canônico os admite e que têm **zero *bindings*** — de modo que **D8-F2** e **D8-F3** são satisfeitas **vacuamente** e **nenhuma decisão factual de runtime** é necessária. **Isto não afirma que S2-D8 reavalie esses fragmentos.** Qualquer mapeamento futuro para fragmento com `RENDERIZADO`, `ASSERTIVA` ou fato de runtime **exige nova arbitragem** — a dupla rota **não abre genericamente** mandatórios com *binding*. |
...
| PE-10 | **Colisão entre as duas origens é *fail-closed***, **exceto** pela dupla rota explicitamente fechada em **PE-13**. Fora dela, um identificador que chegue **pelas duas** significa modelagem incoerente a montante — e isso continua protegendo contra modelar o fragmento de lacuna como alternativa de cobertura: **`R03`/`F1`** por cobertura **mais** a ação de lacuna **fecha**, como antes. **Não existe deduplicação *cross-source* genérica**, duplicatas **não** passaram a ser aceitas, e **nenhum token futuro herda a exceção por analogia**: **PE-13** é um ***owner* explícito**, não um mecanismo. |
