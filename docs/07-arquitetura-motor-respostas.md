...
usar neste ciclo* — já possui materialização (ver **M-DT1–M-DT7** adiante); a **aplicação**
dessa decisão e a **escrita efetiva** do marco **permanecem não implementadas** e pertencem
ao chamador da etapa 13. `src/casa77_sdr/persistence.py`
...
materializadas **depois**, em entrega própria — ver **M-DT1–M-DT7**. Continuam **NÃO
implementadas** a **aplicação** dessa decisão pelo chamador da **etapa 13** e a **escrita
efetiva** do marco, e o **`OrquestradorMotor` continua NÃO implementado**.
...
| 3 | **Transporte e persistência do marco** — continuam sendo o contrato já materializado em **M-T1–M-T6**. A persistência **não decide** quando o marco muda e **não consulta** esta projeção. |
...
| M-T5 | **Zero relógio vivo** e **zero preenchimento automático**: a persistência não cria o marco e **não decide** quando ele muda — essa fronteira permanece intacta. A **decisão** de qual valor usar foi materializada **fora** da persistência (M-DT1–M-DT7); a **aplicação** dessa decisão e a **escrita efetiva** do marco continuam **não implementadas** e pertencem ao chamador da etapa 13. |
...
**APLICAÇÃO** dela pelo chamador da etapa 13 e a **escrita efetiva** via `criar`/`gravar`
...
| M-DT7 | **Permanecem NÃO implementadas**: a **aplicação** da decisão pelo chamador da **etapa 13**, a **escrita efetiva** do marco via `criar`/`gravar`, a **criação operacional** do atendimento, a **integração do pipeline** e o **`OrquestradorMotor`**. **Nenhuma persistência não volátil foi criada.** Portanto **N-a-T3–N-a-T7 NÃO estão operacionalmente concluídas**: apenas a decisão que elas exigem passou a existir em código. |
...
continua PARCIAL**: a **decisão** do marco temporal e a **composição decisória entre as 0–3
chamadas** do ciclo já existem em código (M-DT1–M-DT7), mas a **aplicação** dessa decisão
pela **etapa 13** e a **escrita efetiva** do marco permanecem pendentes — **N-a-T3–N-a-T7
não estão operacionalmente concluídas** —, assim como o **valor do limiar temporal** e
