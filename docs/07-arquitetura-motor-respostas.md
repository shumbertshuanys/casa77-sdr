...
| `MaquinaEstados` | aplicar a ordem do doc 06 §4/§4.2 e a tabela de transições. **Não lê o YAML** e **não fabrica eventos**: consome eventos já confirmados e condições já estruturadas | estado + eventos confirmados + `Qualificacao` + condições já estruturadas (§4.4), **incluindo `insumo_qualificacao_atualizado`** (doc 06 §4.1) | caminho percorrido + **`transicoes_que_mudaram_estado`** (§6.2, contrato arbitrado, **ainda não materializado**) + **estado final único** + ações obrigatórias (§4.5) + efeitos auditáveis |
...
**Arbitragem da projeção de mudança de estado — contrato, não materialização.** As regras
...
origem dessa informação**. Ela **não** é implementação: o campo **não existe em código**,
a `MaquinaEstados` **não o devolve hoje**, **N-a-T3–N-a-T7 continuam NÃO implementadas** e
o **`OrquestradorMotor` continua NÃO implementado**.
...
| `MaquinaEstados` | as **41 transições** do doc 06 §3; a **ordem de avaliação** das famílias C0–C11 (§4.2), com o **caminho percorrido** auditável e **estado final único**; a **projeção `transicoes_que_mudaram_estado`** quando ela for implementada (§6.2 — **contrato arbitrado, implementação funcional futura**); o **fechamento** `E15` → `E12` pós-efeito, o teto de **três chamadas por ciclo** e a ausência de loop; os efeitos paralelos P1–P6 (§4.3) e as inércias N1–N4 (§4.4); evento não coberto por transição, efeito paralelo ou inércia é **erro de contrato** (§4.5); a máquina **não lê o YAML** e **não fabrica eventos** |
