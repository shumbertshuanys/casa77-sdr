...
| D8-N2 | Essa responsabilidade continua **integralmente em `N-b-RES2`**, cujo produtor concreto está **materializado nesta entrega candidata** em `src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não versionado**. **S2-D8 continua não confirmando `E06`**, e o `E09` da combinação continua **fora de RES2**: ele depende das causas de S2-D8 e da integração do ciclo. A **detecção da incoerência permanece exigível** como pré-condição de integração. **Nenhum identificador de pendência novo é criado.** |
...
Permanecem **fora do escopo de S2-D8** e **não resolvidas por ela**: **`N-b-RES2`** — cujo produtor concreto está **materializado nesta entrega candidata** em `src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não versionado**, com a **integração do ciclo ainda pendente**. Permanecem **abertas e inalteradas**: **S3-D1**; **E4**; **E1**; **E3**;
...
ele está **materializado nesta entrega candidata** em
`src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não
versionado**, e ocorre **depois da etapa 4 e fora dela**. Ele **não** é componente — §4.1
...
está **materializado nesta entrega candidata** em `src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não versionado**; e `N-b-RES3` preserva a **classificação** desse residual, **sem criar
...
| M-NB9 | **Fora desta fronteira.** O **produtor não determinístico / LLM da etapa 4** pertence ao **limite único de LLM** de §4.2 e §9 — adaptador, fornecedor, modelo, SDK, API, JSON Schema, *prompt* e interpretação de texto livre **não vivem aqui**. A transformação dos sinais interpretados em **eventos confirmados** pertence a **`N-b-RES2`** — fronteira **posterior e separada**, cujo produtor concreto está **materializado nesta entrega candidata** em `src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não versionado** e que **não vive nesta fronteira determinística**; a etapa 4 continua **proibida de emitir `Exx`** (`N-b-RES1`), com `N-b-RES3` preservado. A **integração operacional** da etapa 4 pertence ao **`OrquestradorMotor`** (D1). **Nenhuma subetapa é criada.** |
...
| M-AJ2-9 | **Fora desta fronteira.** O **produtor não determinístico / LLM da etapa 4** pertence ao **limite único de LLM** de §4.2 e §9 — nenhum fornecedor, modelo, SDK, API, JSON Schema, *prompt* ou interpretação de texto livre vive aqui. `N-b-RES2` é fronteira **posterior e separada**, cujo produtor concreto está **materializado nesta entrega candidata** em `src/casa77_sdr/interpretation_events.py` (§6.3, `RES2-1`–`RES2-12`), ainda **não versionado**, e **não vive nesta fronteira**; a etapa 4 continua **proibida de emitir `Exx`** (`N-b-RES1`, `N-b-RES3`). O **consumo** do `assunto` pertence a **S2-D8** (§4.4.1) e o mapeamento `assunto` → `Rxx` ou → fragmento pertence ao contrato **C** (§2.3): **nenhum dos dois é decidido aqui**. A **integração operacional** pertence ao **`OrquestradorMotor`** (D1). |
...
**11** valores, `AssuntoComercial` com **54** e a lista de erros com **`E-Nb-1`–`E-Nb-19`**.
...
| M-PN3 | **Saída estruturada por JSON Schema** (`output_config.format`), com schema **fechado**: `additionalProperties: false` em **todo** objeto, **todas** as propriedades em `required`, **nove** propriedades na raiz, **zero** parâmetro opcional, **11** parâmetros de união e profundidade máxima **5**. Os enums são **derivados do domínio em tempo de importação** — 2 confianças, 2 formatos, 6 campos, 54 assuntos e 5 códigos autônomos —, **nunca** redigitados. |
...
| M-PN5 | **O schema não possui slot de códigos `A1`** (AJ1-1, M-NB2) e **não possui** `intencoes_detectadas`: o slot autônomo aceita **exatamente** os cinco códigos **A2/B** (AJ1-3). |
...
| RES2-10 | **Eventos explicitamente proibidos** — `E01`, `E07`, `E08`, `E09`, `E11`, `E12`, `E13`, `E14`, `E15`, `E16`, `E17` e `E18`. Em particular: **`E11` e `E17` continuam reduzidos a `E18` pelo `DetectorHandoff`** (doc 06 §2.1, §9), e RES2 **não emite `E18`**; as **causas** de `E09` pertencem a **S2-D8** (§4.4.1, §4.4.3) e **RES2 não confirma `E09`**; `E07`/`E08` pertencem à qualificação e ao ciclo; **`E14` continua em `S3-D1`**. |
