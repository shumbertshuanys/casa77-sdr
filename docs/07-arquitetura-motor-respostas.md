...
| C-14f | as `IntencaoConversacional` — hoje **15**, após **AJ3** —, os erros `E-Nb-1`–`E-Nb-19` e os cenários `K-Nb-1`–`K-Nb-40` (§6.3) |
...
(**`C-P`**): as `IntencaoConversacional` — **15** após **AJ3** — e os erros
...
| C-A1-X5 | `IntencaoConversacional` com **15** valores (**11** à época de C-A1; ampliado por **AJ3**, que C-A1 não reabre) |
...
| C-A2-X5 | `IntencaoConversacional` com **15** valores (**11** à época de C-A2; ampliado por **AJ3**, que C-A2 não reabre) |
...
| D8-C6 | **`IntencaoConversacional` continua fechada** — **15** valores após **AJ3**, que S2-D8 não reabre —, **`E-Nb-1`–`E-Nb-19`** continuam a lista de erros e a `ProjecaoInterpretacao` continua com **sete** campos |
...
| Campo | Conteúdo |
|---|---|
| intenções detectadas (`intencoes_detectadas`) | conjunto de códigos de **`IntencaoConversacional`** — vocabulário conceitual **fechado em 15 valores** (arbitragem N-b, estendida por **AJ3**, adiante), na partição **A1 (6 derivados)**, **A2 (6 autônomos mapeáveis a evento)** e **B (3 autônomos não mapeáveis diretamente a evento)**. O grupo **A2** contém **exatamente seis** sinais — `INTERESSE_EM_VISITA` e `EXCECAO_SOLICITADA`, com **correspondências semânticas** `E10` e `E17`, mais os **quatro sinais de encerramento** de **AJ3**, cuja correspondência semântica é `E14`. **A etapa 4 não emite `Exx`**: a transformação desses sinais em evento confirmado ocorre **depois da etapa 4 e fora dela**, com produtores concretos **materializados** em `src/casa77_sdr/interpretation_events.py` (`RES2-1`–`RES2-12`, adiante) e, para `E14` e `motivo_encerramento`, em `src/casa77_sdr/closure_decision.py` (**S3-D1**, `S3D1-1`–`S3D1-12`, adiante); a **integração do ciclo continua pendente** (N-b-G2, N-b-RES1–N-b-RES3) |
...
**`IntencaoConversacional` — vocabulário conceitual fechado com exatamente 15 valores**
(**11** na arbitragem N-b original; **A2** ampliado de **dois** para **seis** por **AJ3**).
...
| **A2 — autônomos mapeáveis a evento (6)** | 7 | `INTERESSE_EM_VISITA` | autônomo |
...
| **B — autônomos não mapeáveis diretamente a evento (3)** | 13 | `INTERESSE_CONFIRMAR_DISPONIBILIDADE` | autônomo; alimenta a **condição 5** de §4.4 |
| | 14 | `CONTINUIDADE_DE_EVENTO_DECLARADA` | autônomo; alimenta `intencao_identidade` |
| | 15 | `EVENTO_NOVO_DECLARADO` | autônomo; alimenta `intencao_identidade` |
...
`IntencaoConversacional` permanece fechada — **15** valores após **AJ3**, que AJ1 não
reabre; a lista de erros de contrato
...
| AJ1-3 | O **slot de intenções autônomas** aceita **exatamente** os códigos **A2/B** — cinco à época de AJ1 e **nove** após **AJ3**: `INTERESSE_EM_VISITA`, `EXCECAO_SOLICITADA`, `DESINTERESSE_DECLARADO`, `CONTATO_POR_ENGANO`, `MENSAGEM_NAO_SOLICITADA`, `ACEITACAO_DE_INCOMPATIBILIDADE`, `INTERESSE_CONFIRMAR_DISPONIBILIDADE`, `CONTINUIDADE_DE_EVENTO_DECLARADA` e `EVENTO_NOVO_DECLARADO`. |
...
| M-NB3 | A entrada da fronteira é uma representação **pré-canônica mínima** que carrega **somente** o que o produtor não determinístico entrega (AJ1-2) mais o **slot autônomo** fechado nos **nove** códigos **A2/B** (AJ1-3, ampliado por **AJ3**). Ela **não é uma segunda `Interpretacao`** e **nenhum formato de transporte é escolhido** (N-b-F3). |
...
**§4.1 permanece com 14 componentes** e §2 com **nove** responsabilidades.
`IntencaoConversacional` permanece fechada — **15** valores após **AJ3**, que AJ2 não
reabre —, a lista de erros permanece
...
**`K-Nb-1`–`K-Nb-51`** (**`C-P`**). As `IntencaoConversacional` — **15** após **AJ3** — e os erros
...
| M-AJ2-6 | **Fronteira preservada.** A `ProjecaoInterpretacao` continua com **sete** campos e o `assunto` **não atravessa** para ela (N-b-Q12, N-b-K8); `IntencaoConversacional` continua fechada — **15** valores após **AJ3**, que M-AJ2 não reabre; a **condição 5** continua a **única** condição de §4.4 produzida por esta fronteira; as condições **2** e **4** possuem **produtor conceitual** atribuído por **S2-D8** (§4.4.1, eixos **A** e **B**) e **ficam fora desta fronteira**; a condição **8** tem produtor em **S3-D1** e **fica igualmente fora desta fronteira**; e o `assunto` **não participa de N-b-X3**: a agregação de `PERGUNTA_COMERCIAL` continua dependendo **somente** das confianças. O `assunto` **não referencia `Rxx`**, **não seleciona fragmento** e **não produz `Exx`**. |
...
| D8-Nb6 | **Fronteira preservada**: `IntencaoConversacional` continua fechada — **15** valores após **AJ3** —, `AssuntoComercial` com **54**, a lista de erros com **`E-Nb-1`–`E-Nb-19`**, os cenários com **`K-Nb-1`–`K-Nb-51`**, a `ProjecaoInterpretacao` com **sete** campos, §4.1 com **14** componentes e §2 com **nove** responsabilidades. |
...
— **15** valores após **AJ3** —, `AssuntoComercial` com **54** e a lista de erros com
**`E-Nb-1`–`E-Nb-19`**.
...
`src/casa77_sdr/interpretation.py` permanece **inalterado** e continua a autoridade.
...
| M-PN3 | **Saída estruturada por JSON Schema** (`output_config.format`), com schema **fechado**: `additionalProperties: false` em **todo** objeto, **todas** as propriedades em `required`, **nove** propriedades na raiz, **zero** parâmetro opcional, **11** parâmetros de união e profundidade máxima **5**. Os enums são **derivados do domínio em tempo de importação** — 2 confianças, 2 formatos, 6 campos, 54 assuntos e **9** códigos autônomos (**AJ3**) —, **nunca** redigitados. Os **11** parâmetros de união são **estrutura de union types**, não a cardinalidade de `IntencaoConversacional`: ampliar o enum autônomo **não** os altera. |
...
| M-PN5 | **O schema não possui slot de códigos `A1`** (AJ1-1, M-NB2) e **não possui** `intencoes_detectadas`: o slot autônomo aceita **exatamente** os **nove** códigos **A2/B** (AJ1-3, ampliado por **AJ3**). |
...
| RES2-10 | **Eventos explicitamente proibidos** — `E01`, `E07`, `E08`, `E09`, `E11`, `E12`, `E13`, `E14`, `E15`, `E16`, `E17` e `E18`. Em particular: **`E11` e `E17` continuam reduzidos a `E18` pelo `DetectorHandoff`** (doc 06 §2.1, §9), e RES2 **não emite `E18`**; as **causas** de `E09` pertencem a **S2-D8** (§4.4.1, §4.4.3) e **RES2 não confirma `E09`**; `E07`/`E08` pertencem à qualificação e ao ciclo; **`E14` pertence a `S3-D1`** (§6.3, `S3D1-1`–`S3D1-12`), fronteira **posterior e separada**, e **RES2 não muda por causa dela**. |
...
| RES2-12 | **Pureza e fronteira.** Zero I/O, filesystem, rede, relógio, YAML, `knowledge/**`, LLM, SDK, persistência, logging, cache, retry e *sleep*; a entrada **não é mutada**. RES2 **não produz** condição de §4.4, qualificação, ação, resposta, *handoff* nem persistência, e **não faz a integração do ciclo**. Unir eventos vindos de **produtores distintos** — RES2, S2-D8, `DetectorHandoff`, `Qualificador` — é papel do **`OrquestradorMotor` futuro**, que **continua ausente**. A compatibilidade estrutural com a `MaquinaEstados` é provada sobre os **63 subconjuntos não vazios × 8 estados**, com condições **neutras e coerentes**; a pré-condição **D8-N1** é provada **separadamente**, e o `E09` que ela exige **não é produzido por RES2**. |
...
| # | Decisão AJ3 |
|---|---|
| AJ3-1 | `IntencaoConversacional` passa de **11** para **15** valores. A partição é preservada: **A1 = 6 derivados**; **A2 = 6 autônomos mapeáveis a evento**; **B = 3 autônomos não mapeáveis diretamente a evento**. |
...
| 12 | **N-b** — contrato global da **interpretação** da etapa 4: quem produz a projeção estruturada de §6.3 e com que garantias | **ARBITRADA** (§6.3): contrato da **`Interpretacao`** fechado — as **oito** categorias de §6.3; **`IntencaoConversacional`** com **exatamente 15** códigos na partição **A1 (6 derivados) / A2 (6 autônomos) / B (3 autônomos)** — **11** na arbitragem original, **estendida por AJ3** (item 23); consistência cruzada **N-b-X1–N-b-X6**; regras de confiança **N-b-G6/G6b/G6c**; lista fechada de erros **E-Nb-1–E-Nb-19**; modo degradado **N-b-M1–N-b-M8**; fronteira do produtor **N-b-F1–N-b-F5**; e cenários **K-Nb-1–K-Nb-51** (§8.2), fronteira estendida por **AJ2** (item 20). **Produtor atribuído**: o produtor de interpretação da etapa 4 é **fronteira funcional** do limite único de LLM (§4.2, §9), **não componente novo** — §4.1 permanece com **14** componentes. **AJ1** (§6.3, §8.2) fecha a **representação e a canonicalização determinística** de N-b. **Residual de contrato**: a etapa 4 **continua não emitindo `Exx`** (**N-b-RES1**, **N-b-RES3** inalteradas), e a transformação dos sinais interpretados em **eventos confirmados** — que era o residual **`N-b-RES2`** — passa a ter **produtor concreto**: **`N-b-RES2` MATERIALIZADO E VERSIONADO** (§6.3, `RES2-1`–`RES2-12`), em `src/casa77_sdr/interpretation_events.py`, com saída fechada em **`E02`/`E03`/`E04`/`E05`/`E06`/`E10`** | **contrato resolvido** — §6.3. A **fronteira determinística** é **M-NB1–M-NB9** e **M-AJ2-1–M-AJ2-9**; o **produtor não determinístico / LLM** é **M-PN1–M-PN12**; o **produtor de eventos derivados** é **`RES2-1`–`RES2-12`**. **Os demais produtores de evento permanecem ausentes** e **não são resolvidos aqui**: `E09` depende da integração das causas de **S2-D8** (item 10), `E18` do **`DetectorHandoff`** (doc 06 §9) e `E07`/`E08` da qualificação e do ciclo. **`E14` deixou de ser lacuna**: o seu produtor é **`S3-D1`**, materializado em `src/casa77_sdr/closure_decision.py` (§6.3, `S3D1-1`–`S3D1-12`), **fora desta fronteira**. A **integração da etapa 4** e a **união dos eventos de produtores distintos** continuam pertencendo ao `OrquestradorMotor` |
