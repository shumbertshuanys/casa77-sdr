...
artefato **legível**: existência, leitura, decodificação em **UTF-8 estrito**,
análise **YAML segura** e recusa de **chave repetida** no mesmo mapeamento. Toda
a **forma** — esqueleto fechado, vocabulário de assunto, cardinalidades de
`R2-1` e `R2-2`, `R2-6` e a proibição do fragmento de lacuna como alternativa —
pertence a `coverage_map.validar_mapa_cobertura`, que é chamado sobre a
...
volta é exatamente a estrutura que o analisador produziu, na **ordem física
declarada**.
...
`MapaCoberturaIlegivel` carrega **categoria e caminho** e nada mais — nunca o
conteúdo do arquivo, o valor recebido ou o texto bruto do analisador. A causa
técnica original, quando existe, fica encadeada em `__cause__`.
"""
...
from casa77_sdr.coverage_map import validar_mapa_cobertura

__all__ = ["MapaCoberturaIlegivel", "carregar_mapa_cobertura"]


class MapaCoberturaIlegivel(Exception):
    ...
    isso é `MapaCoberturaInvalido`, levantada por `coverage_map`.
    """


# Categorias fechadas de ilegibilidade. Nenhuma delas descreve conteudo.
...
def carregar_mapa_cobertura(path: str | Path) -> dict[str, Any]:
    """Lê `path`, analisa o YAML e devolve a estrutura validada por `R2`.
    ...
    **somente** em UTF-8. A análise usa exclusivamente `_LeitorEstrito`, que é
    ...
    A estrutura analisada é entregue a `validar_mapa_cobertura(...)` **sem
    retoque**: a raiz chega como o analisador a produziu, inclusive quando é
    `None`, lista ou escalar, e é o contrato estrutural de `R2` quem a rejeita.
    `MapaCoberturaInvalido` **propaga intacta**, com categoria e localizador
    originais — ela **não** é capturada, traduzida ou escondida.

    Levanta `MapaCoberturaIlegivel` quando o artefato não pode ser lido ou
...
def _ilegivel(categoria: str, caminho: Path) -> MapaCoberturaIlegivel:
    return MapaCoberturaIlegivel(f"{categoria}: {caminho}")
